...
class DataValidator:
    ...
    def validate_no_null_values(self, df: pd.DataFrame, columns: Optional[List[str]] = None) -> None:
    ...
    def validate_numeric_range(self, df: pd.DataFrame, column: str, min_val: float, max_val: float) -> None:
        ...
        if not out_of_range.empty:
            raise ValidationError(f"Column '{column}' has {len(out_of_range)} values outside range [{min_val}, {max_val}]")
