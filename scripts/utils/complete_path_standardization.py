...
class CompletePathStandardizer:
    ...
    def validate_completion(self) -> Dict[str, List[str]]:
        ...
        problem_patterns = [
            ...
            (r'(?<!\{SCRIPTS_BASE\}/)(?<!\{SCRIPTS_BASE\})scripts/', 'Hardcoded scripts/ reference'),
            
            # Templates references (excluding already correct ones)  
            (r'(?<!\{TEMPLATES_BASE\}/)(?<!\{TEMPLATES_BASE\})templates/', 'Hardcoded templates/ reference'),
            
            # Data outputs references (excluding already correct ones)
            (r'(?<!\{DATA_OUTPUTS\}/)(?<!\{DATA_OUTPUTS\})data/outputs/', 'Hardcoded data/outputs/ reference'),
