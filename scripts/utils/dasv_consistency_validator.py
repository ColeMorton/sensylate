...
class DASVConsistencyValidator:
    ...
    def _load_system_configuration(self):
        ...
        self.system_standards = {
            ...
            'analysis_domains': [
                'fundamental_analysis', 'sector_analysis', 'industry_analysis',
                'comparative_analysis', 'macro_analysis', 'trade_history'
            ],
            'required_path_variables': [
                'SCRIPTS_BASE', 'TEMPLATES_BASE', 'DATA_OUTPUTS', 'SCHEMAS_BASE'
            ],
    ...
    def validate_template_consistency(self) -> ValidationResult:
        ...
        base_template_usage = sum(1 for analysis in self.template_optimizer.template_analyses 
                                if any('base_' in inc for inc in analysis.includes_extends))
        base_usage_rate = base_template_usage / template_report.total_templates if template_report.total_templates > 0 else 0
        
        if base_usage_rate < 0.6:
            issues.append(f"Low base template usage: {base_usage_rate:.1%}")
        
        # Check macro consistency
        templates_with_macros = sum(1 for analysis in self.template_optimizer.template_analyses 
                                  if analysis.macro_imports)
        macro_usage_rate = templates_with_macros / template_report.total_templates if template_report.total_templates > 0 else 0
            ...
            if domain in command_mappings:
                ...
                for phase in self.system_standards['dasv_phases']:
                    if phase in domain_phases:
                        ...
                        required_fields = ['sub_agent', 'primary_script', 'schema', 'output_dir', 'file_pattern']
    ...
    def validate_quality_standards_compliance(self) -> ValidationResult:
        ...
        confidence_coverage = schemas_with_confidence / len(self.schema_optimizer.schema_analyses) if self.schema_optimizer.schema_analyses else 0
        ...
        templates_with_quality = sum(1 for analysis in self.template_optimizer.template_analyses 
                                   if analysis.quality_indicators)
        quality_coverage = templates_with_quality / template_report.total_templates if template_report.total_templates > 0 else 0
    ...
    def run_comprehensive_validation(self) -> SystemConsistencyReport:
        ...
        summary = {
            ...
            'system_health': "excellent" if overall_score >= 0.9 else 
                           ...
                           "needs_attention" if overall_score >= 0.7 else "critical"
        }
        ...
        if overall_score < self.thresholds['overall_system_minimum']:
            drift_indicators.append(f"Overall system score {overall_score:.2f} below minimum {self.thresholds['overall_system_minimum']}")
...
def main():
    ...
    parser.add_argument('--check', choices=[
        'schemas', 'templates', 'commands', 'quality', 'execution', 'all'
    ...
    parser.add_argument('--quiet', action='store_true',
                       help='Minimize output (scores only)')
