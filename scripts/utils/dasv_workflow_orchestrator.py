...
from command_execution_service import CommandExecutionService, ExecutionResult, ExecutionStatus, ExecutionMode
...
class DASVWorkflowOrchestrator:
        ...
        return {
            ...
            'industry_analysis': {
                ...
                'quality_gates': {
                    ...
                    'analyze': ['competitive_moat_assessment', 'growth_catalyst_validation'],
    ...
    def _evaluate_quality_gate(self, phase: str, execution_result: ExecutionResult, 
                              domain: str) -> Tuple[QualityGateResult, List[str]]:
        ...
        if execution_result.confidence_score is not None:
            if execution_result.confidence_score < self.quality_thresholds['minimum_confidence']:
                issues.append(f"Confidence score {execution_result.confidence_score:.2f} below minimum {self.quality_thresholds['minimum_confidence']}")
    ...
    def execute_workflow(self, domain: str, parameters: Dict[str, Any], 
        ...
        try:
            ...
            for i, phase in enumerate(phases):
                ...
                status_emoji = "✅" if quality_result == QualityGateResult.PASS else "⚠️" if quality_result == QualityGateResult.WARNING else "❌"
            ...
            if workflow_result.status == WorkflowStatus.IN_PROGRESS:
                failed_phases = [p for p, r in workflow_result.phase_results.items() 
                               if r.execution_result.status == ExecutionStatus.FAILED]
                warning_phases = [p for p, r in workflow_result.phase_results.items() 
            ...
            confidences = [r.confidence_score for r in workflow_result.phase_results.values() 
            ...
            # Final summary
            status_emoji = "🎉" if workflow_result.status == WorkflowStatus.SUCCESS else "⚠️" if workflow_result.status == WorkflowStatus.PARTIAL_SUCCESS else "💥"
...
def main():
    ...
    for phase, phase_result in result.phase_results.items():
        ...
        quality_emoji = "✅" if phase_result.quality_gate_result == QualityGateResult.PASS else "⚠️" if phase_result.quality_gate_result == QualityGateResult.WARNING else "❌"
        
        print(f"  {status_emoji} {phase}: {phase_result.execution_result.status.value} (Quality: {quality_emoji} {phase_result.quality_gate_result.value})")
