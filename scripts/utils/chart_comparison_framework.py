...
class ChartComparisonFramework:
    ...
    def _create_side_by_side_comparison(
        ...
        draw.text((img_mpl.width + 20 + img_plotly.width // 2, 60), "Plotly", fill='black', font=font, anchor='mb')
    ...
    def generate_test_report(self, comparison_results: List[Dict[str, Any]]) -> str:
        ...
        with open(report_path, 'w') as f:
            ...
            avg_similarity = np.mean([r["similarity_metrics"]["similarity_percent"] 
            ...
            for result in comparison_results:
                ...
                f.write(f"- **JSON Schema**: [{Path(result['schema_path']).name}]({Path(result['schema_path']).name})\n\n")
...
def create_test_data() -> Dict[str, Any]:
    ...
    monthly_data = [
        MonthlyPerformance(month="January", year=2024, trades_closed=12, win_rate=65.5, average_return=2.3, market_context="Bullish"),
        MonthlyPerformance(month="February", year=2024, trades_closed=8, win_rate=58.2, average_return=-0.8, market_context="Bearish"),
        MonthlyPerformance(month="March", year=2024, trades_closed=15, win_rate=72.1, average_return=3.5, market_context="Volatile"),
        MonthlyPerformance(month="April", year=2024, trades_closed=10, win_rate=61.8, average_return=1.2, market_context="Sideways"),
        MonthlyPerformance(month="May", year=2024, trades_closed=13, win_rate=69.3, average_return=2.7, market_context="Bullish"),
        MonthlyPerformance(month="June", year=2024, trades_closed=9, win_rate=55.7, average_return=-1.1, market_context="Bearish"),
    ]
    
    # Quality distribution data
    quality_data = [
        QualityDistribution(category="Excellent", count=15, percentage=25.5, win_rate=85.2, average_return=4.2),
        QualityDistribution(category="Good", count=21, percentage=35.8, win_rate=68.5, average_return=2.1),
        QualityDistribution(category="Poor", count=12, percentage=20.3, win_rate=45.7, average_return=-0.8),
        QualityDistribution(category="Failed", count=11, percentage=18.4, win_rate=22.1, average_return=-3.5),
    ]
