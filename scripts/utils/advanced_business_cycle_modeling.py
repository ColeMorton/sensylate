...
class AdvancedBusinessCycleEngine:
    ...
    def __init__(self, region: str = "US"):
        ...
        self.regime_thresholds = {
            ...
            'recession': {'gdp_max': 0.5, 'unemployment_min': 5.5, 'volatility_min': 20}
        }
    ...
    def analyze_advanced_business_cycle(self, discovery_data: Dict[str, Any], 
    ...
    def _extract_cycle_indicators(self, discovery_data: Dict, analysis_data: Dict) -> Dict[str, Any]:
        ...
        # GDP data
        gdp_data = discovery_data.get("cli_comprehensive_analysis", {}).get(
            "central_bank_economic_data", {}).get("gdp_data", {})
        current_gdp = 2.0  # Default
        if gdp_data.get("observations"):
            current_gdp = gdp_data["observations"][0].get("value", 2.0)
            
        # Employment data
        employment_data = discovery_data.get("cli_comprehensive_analysis", {}).get(
            "central_bank_economic_data", {}).get("employment_data", {})
        ...
        yield_spread = yield_curve.get("current_spread", {}).get("10y_2y", 0.5) if isinstance(yield_curve.get("current_spread"), dict) else 0.5
    ...
    def _adjust_transition_probabilities(self, indicators: Dict[str, Any]) -> MarkovTransitionMatrix:
        ...
        duration_factor = min(indicators["phase_duration_months"] / 24.0, 2.0)  # Longer phases more likely to transition
    ...
    def _calculate_multi_period_transitions(self, current_phase: str, 
                                          transitions: MarkovTransitionMatrix) -> Dict[str, Any]:
        ...
        results = {}
        for period, matrix, label in [(1, P1, "1_quarter"), (2, P2, "2_quarters"), 
                                     (4, P4, "1_year"), (8, P8, "2_years")]:
    ...
    def _analyze_transition_timing(self, indicators: Dict[str, Any], 
                                 transitions: MarkovTransitionMatrix) -> Dict[str, Any]:
        ...
        if current_phase == "expansion":
            ...
            expected_quarters_to_recession = 1.0 / (peak_prob + recession_prob) if (peak_prob + recession_prob) > 0 else float('inf')
            ...
            return {
                ...
                "phase_maturity_assessment": "early" if phase_duration < 12 else "mid" if phase_duration < 36 else "late",
                "transition_urgency": "low" if expected_quarters_to_peak > 8 else "moderate" if expected_quarters_to_peak > 4 else "high"
            }
            ...
            expected_quarters_to_resolution = 1.0 / (contraction_prob + expansion_prob) if (contraction_prob + expansion_prob) > 0 else 2.0
            
            return {
                "current_phase_expected_remaining_quarters": round(expected_quarters_to_resolution, 1),
                "soft_landing_probability": round(expansion_prob / (contraction_prob + expansion_prob), 3) if (contraction_prob + expansion_prob) > 0 else 0.5,
                "recession_probability": round(contraction_prob / (contraction_prob + expansion_prob), 3) if (contraction_prob + expansion_prob) > 0 else 0.5,
            ...
            expected_quarters_to_recovery = 1.0 / (trough_prob + recovery_prob) if (trough_prob + recovery_prob) > 0 else 6.0
            ...
            return {
                ...
                "v_shaped_recovery_probability": round(recovery_prob / (trough_prob + recovery_prob), 3) if (trough_prob + recovery_prob) > 0 else 0.25,
    ...
    def _identify_transition_risk_factors(self, indicators: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            ...
            "key_monitoring_indicators": ["yield_curve_slope", "recession_probability", "gdp_growth", "unemployment_rate"]
        }
    ...
    def _detect_regime_switches(self, indicators: Dict[str, Any]) -> Dict[str, Any]:
        ...
        switching_probability = self._calculate_regime_switching_probability(current_regime, regime_duration, indicators)
    ...
    def _calculate_regime_probabilities(self, gdp_growth: float, unemployment: float, volatility: float) -> Dict[str, float]:
    ...
    def _describe_regime_characteristics(self, regime: str) -> Dict[str, Any]:
        ...
        characteristics = {
            ...
            "stagnation": {
                ...
                "key_risks": ["Deflation", "Secular stagnation", "Political instability"]
            },
            "recession": {
    ...
    def _analyze_sector_rotation(self, indicators: Dict[str, Any], 
    ...
    def _calculate_rotation_probability(self, indicators: Dict[str, Any], 
                                      markov_analysis: Dict[str, Any]) -> float:
    ...
    def _determine_rotation_timing(self, indicators: Dict[str, Any], 
                                 markov_analysis: Dict[str, Any]) -> str:
    ...
    def _identify_rotation_catalysts(self, indicators: Dict[str, Any], 
                                   markov_analysis: Dict[str, Any]) -> List[str]:
    ...
    def _identify_contrarian_opportunities(self, cycle_stage: str) -> List[str]:
        ...
        contrarian_map = {
            ...
            "mid_expansion": ["technology", "consumer_discretionary"],  # Early positioning
            ...
            "mid_contraction": ["consumer_discretionary", "materials"],  # Recovery positioning
    ...
    def _calculate_multi_horizon_recession_probability(self, indicators: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            ...
            "key_drivers": ["yield_curve", "employment_trends", "gdp_momentum", "financial_conditions"],
    ...
    def _get_volatility_investment_implications(self, regime: str) -> Dict[str, Any]:
        ...
        implications = {
            "ultra_low_volatility": {
                ...
                "asset_preferences": ["low_vol_stocks", "covered_calls", "credit_spreads"]
            },
            "low_volatility": {
            ...
            "extreme_volatility": {
                ...
                "asset_preferences": ["cash", "treasuries", "quality_stocks_at_discount"]
            }
        }
        
        return implications.get(regime, {})
        
    def _create_integrated_assessment(self, markov_analysis: Dict[str, Any],
    ...
    def _generate_integrated_investment_implications(self, markov_analysis: Dict[str, Any],
                                                   regime_analysis: Dict[str, Any],
                                                   rotation_analysis: Dict[str, Any],
                                                   overall_risk_score: float) -> Dict[str, Any]:
        ...
        return {
            ...
            "volatility_positioning": "hedge" if regime_analysis["volatility_regime"] == "low_volatility" else "opportunistic",
            "rebalancing_urgency": markov_analysis["transition_timing_analysis"].get("transition_urgency", "moderate")
        }
        
    def _calculate_integrated_confidence(self, markov_analysis: Dict[str, Any],
                                       regime_analysis: Dict[str, Any], 
                                       rotation_analysis: Dict[str, Any],
    ...
    def _identify_risk_monitoring_priorities(self, markov_analysis: Dict[str, Any],
    ...
    def _generate_tactical_recommendations(self, rotation_analysis: Dict[str, Any],
        ...
        return {
            ...
            "volatility_management": {
                ...
                "hedging_recommendations": "moderate" if regime_analysis.get("volatility_regime") == "low_volatility" else "defensive"
            }
        }
