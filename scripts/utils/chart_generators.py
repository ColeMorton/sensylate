...
from scripts.utils.dashboard_parser import TradeData, MonthlyPerformance, QualityDistribution
...
class AdvancedChartGenerator:
    ...
    def create_enhanced_gauge(self, ax: plt.Axes, value: float, title: str, 
                            max_value: float = 100, mode: str = 'light') -> None:
        """
        ...
        # Background arc
        bg_arc = patches.Arc((0, 0), 2, 2, angle=0, theta1=0, theta2=180,
                           linewidth=8, color=theme.borders, alpha=0.3)
        ax.add_patch(bg_arc)
        
        # Value arc
        value_angle = (value / max_value) * 180
        value_arc = patches.Arc((0, 0), 2, 2, angle=0, theta1=0, theta2=value_angle,
                              linewidth=8, color=self.theme_manager.color_palette.primary_data)
        ax.add_patch(value_arc)
        
        # Center circle
        center = Circle((0, 0), 0.1, facecolor=theme.card_backgrounds, 
                       edgecolor=theme.borders, linewidth=2)
        ax.add_patch(center)
        
        # Value text
        ax.text(0, -0.3, f"{value:.1f}%", ha='center', va='center',
               fontsize=14, fontweight='bold', color=theme.primary_text)
        
        # Title
        ax.text(0, -0.5, title, ha='center', va='center',
               fontsize=10, color=theme.body_text)
    
    def create_enhanced_monthly_bars(self, ax: plt.Axes, monthly_data: List[MonthlyPerformance], 
                                   mode: str = 'light') -> None:
        """
        ...
        bars = ax.bar(x_pos, win_rates, color=colors[:len(months)], 
                     alpha=0.8, edgecolor=theme.borders, linewidth=1)
        ...
        for i, (bar, rate, ret) in enumerate(zip(bars, win_rates, returns)):
            ...
            ax.text(bar.get_x() + bar.get_width()/2., height + 2,
                   ...
                   fontsize=9, fontweight='bold', color=theme.primary_text)
            
            # Return label (smaller, below)
            return_color = (self.theme_manager.color_palette.primary_data 
                          if ret >= 0 else self.theme_manager.color_palette.secondary_data)
            ax.text(bar.get_x() + bar.get_width()/2., height/2,
                   f'{ret:+.1f}%', ha='center', va='center',
                   fontsize=8, color=return_color, fontweight='bold')
    ...
    def create_enhanced_donut_chart(self, ax: plt.Axes, quality_data: List[QualityDistribution], 
                                  mode: str = 'light') -> None:
        """
        ...
        for i, (wedge, category, percentage, win_rate) in enumerate(zip(wedges, categories, percentages, win_rates)):
    ...
    def create_waterfall_chart(self, ax: plt.Axes, trades: List[TradeData], 
        ...
        # Cumulative line
        final_cumulative = cumulative + returns
        ax.plot(range(len(returns)), final_cumulative, 
        ...
        if self.scalability_manager:
            ...
            ax.set_xticklabels([tickers[i] for i in range(0, len(trades), label_freq)], 
                              rotation=45, ha='right', fontsize=8)
        else:
            step = max(1, len(trades) // 8)
            ax.set_xticks(range(0, len(trades), step))
            ax.set_xticklabels([tickers[i] for i in range(0, len(trades), step)], 
                              rotation=45, ha='right', fontsize=8)
        
        ax.grid(True, alpha=0.3, axis='y')
        ax.tick_params(colors=theme.body_text)
    
    def _create_performance_bands_chart(self, ax: plt.Axes, trades: List[TradeData], 
                                      mode: str = 'light') -> None:
        """
        ...
        bars = ax.barh(y_pos, band_counts, color=band_colors, alpha=0.8,
                      edgecolor=theme.borders, linewidth=1)
        
        # Add count labels
        for i, (bar, count) in enumerate(zip(bars, band_counts)):
            ax.text(bar.get_width() + 0.1, bar.get_y() + bar.get_height()/2,
                   f'{count}', ha='left', va='center', fontsize=10,
                   color=theme.primary_text, fontweight='bold')
    ...
    def create_enhanced_scatter(self, ax: plt.Axes, trades: List[TradeData], 
                              mode: str = 'light') -> None:
        """
        ...
        # Color and size by quality and return magnitude
        colors = []
        sizes = []
        alphas = []
        ...
        for trade in trades:
            ...
            # Size based on absolute return magnitude
            sizes.append(50 + abs(trade.return_pct) * 3)
            # Alpha based on quality with density adjustment
            quality_alpha = {'Excellent': 0.9, 'Good': 0.8, 'Poor': 0.6, 
                            'Failed': 0.5, 'Poor Setup': 0.4}.get(trade.quality, 0.6)
            alphas.append(quality_alpha * base_alpha)
        
        # Create scatter plot with enhanced styling
        for i, (dur, ret, color, size, alpha) in enumerate(zip(durations, returns, colors, sizes, alphas)):
            ax.scatter(dur, ret, c=color, s=size, alpha=alpha, 
                      edgecolors=theme.borders, linewidth=0.5)
        ...
        # Quadrant labels
        max_dur = max(durations) if durations else 30
        max_ret = max(returns) if returns else 10
        min_ret = min(returns) if returns else -10
        
        # Quick wins quadrant (top-left)
        ax.text(max_dur * 0.1, max_ret * 0.8, 'Quick\nWins', 
               ha='center', va='center', fontsize=9, 
               color=theme.body_text, alpha=0.6, style='italic')
        
        # Patient winners quadrant (top-right)
        ax.text(max_dur * 0.8, max_ret * 0.8, 'Patient\nWinners', 
               ha='center', va='center', fontsize=9,
               color=theme.body_text, alpha=0.6, style='italic')
        ...
        for quality, color in quality_colors.items():
            if any(trade.quality == quality for trade in trades):
                legend_elements.append(plt.scatter([], [], c=color, s=60, alpha=0.8, 
                                                 label=quality, edgecolors=theme.borders))
        
        if legend_elements:
            ax.legend(handles=legend_elements, loc='upper right', fontsize=8,
                     framealpha=0.9, facecolor=theme.card_backgrounds)
    
    def _create_clustered_scatter(self, ax: plt.Axes, cluster_info: Dict[str, Any], 
                                mode: str = 'light') -> None:
        """
        ...
        for cluster in cluster_info['clusters']:
            ...
            # Plot centroid
            ax.scatter(centroid_dur, centroid_ret, c=color, s=marker_size, 
                      alpha=0.7, edgecolors=theme.borders, linewidth=2,
                      marker='o')
            
            # Add cluster size label
            ax.text(centroid_dur, centroid_ret, str(cluster_size), 
                   ha='center', va='center', fontsize=8,
                   color='white', fontweight='bold')
        ...
        if cluster_info['noise']:
            ...
            for dur, ret, color in zip(noise_durations, noise_returns, noise_colors):
                ax.scatter(dur, ret, c=color, s=30, alpha=0.6,
                          edgecolors=theme.borders, linewidth=0.5)
        ...
        ax.text(0.02, 0.98, stats_text, transform=ax.transAxes,
               ...
               bbox=dict(boxstyle="round,pad=0.3", 
                        ...
                        edgecolor=theme.borders, alpha=0.8))
    ...
    def create_performance_summary_panel(self, ax: plt.Axes, trades: List[TradeData],
...
Worst Month: {worst_month.month if monthly_data else 'N/A'}"""
            ...
            ax.text(0.05, 0.95, summary_text, 
                   ...
                   color=theme.body_text, family='monospace',
                   bbox=dict(boxstyle="round,pad=0.3", 
                           facecolor=theme.card_backgrounds,
                           edgecolor=theme.borders, alpha=0.8))
