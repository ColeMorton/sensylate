...
from scripts.utils.command_execution_service import (
    CommandExecutionService, ExecutionMode, ExecutionStatus
)
from scripts.utils.dasv_workflow_orchestrator import (
    DASVWorkflowOrchestrator, WorkflowStatus
)
...
class UnifiedCommandInterface:
    ...
    def _validate_command_parameters(self, domain: str, phase: str, 
    ...
    def execute_single_phase(self, domain: str, phase: str, parameters: Dict[str, Any],
                           mode: ExecutionMode = ExecutionMode.DIRECT) -> Dict[str, Any]:
        """
    ...
    def execute_full_workflow(self, domain: str, parameters: Dict[str, Any],
    ...
    def get_command_info(self, domain: str, phase: Optional[str] = None) -> Dict[str, Any]:
        ...
        if phase:
            ...
            if domain in available_commands:
                return {
                    'domain': domain,
                    'phases': available_commands[domain]
                }
            else:
            ...
            if len(parts) == 2 and '=' not in parts[1]:
                ...
                for param in parts[1:]:
                    if '=' in param:
                        ...
                        if domain in ['fundamental_analysis', 'fa', 'trade_history', 'th'] and 'ticker' not in parameters:
...
Examples:
  ...
  %(prog)s --info fa                          # get info about fundamental_analysis
    ...
    parser.add_argument('--info', metavar='DOMAIN',
    ...
    if 'error' in result:
        ...
        if 'workflow_id' in result:
            ...
            for phase, phase_info in result.get('phase_results', {}).items():
                ...
                print(f"  {status_emoji} {phase}: {phase_info['status']} (Quality: {quality_emoji} {phase_info['quality_gate']})")
