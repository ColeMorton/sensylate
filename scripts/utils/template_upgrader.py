...
{% endblock %}""",
    ...
    def _check_metadata_structure(self, content: str, plan: TemplateUpgradePlan):
        ...
        if 'metadata' in content:
            ...
            required_fields = ['command_name', 'execution_timestamp', 'framework_phase', 'domain_identifier']
    ...
    def _check_block_structure(self, content: str, plan: TemplateUpgradePlan):
        ...
        expected_blocks = {
            'analysis': ['executive_summary', 'key_findings', 'detailed_analysis', 'investment_recommendation'],
            'discovery': ['data_summary', 'key_metrics', 'cli_validation', 'data_quality_assessment'],
            'synthesis': ['investment_thesis', 'portfolio_allocation', 'risk_profile', 'action_items'],
            'validation': ['validation_summary', 'quality_certification', 'audit_trail', 'confidence_assessment']
        }
    ...
    def upgrade_all_templates(self, dry_run: bool = True, pattern: str = "*.j2") -> UpgradeReport:
        ...
        for template_file in self.templates_dir.rglob(pattern):
            ...
            # Upgrade template
            print(f"  {'🔍' if dry_run else '🔧'} {template_file.relative_to(self.templates_dir)} - {len(plan.upgrades_required)} upgrades")
...
def main():
    ...
    parser.add_argument('--pattern', default='*.j2',
                       help='File pattern to process (default: *.j2)')
