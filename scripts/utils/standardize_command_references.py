...
class CommandReferenceStandardizer:
    ...
    def validate_standardization(self) -> Dict[str, List[str]]:
        ...
        problem_patterns = [
            ...
            (r'(?<!\{SCRIPTS_BASE\})\/scripts\/', 'Hardcoded scripts/ reference (should be {SCRIPTS_BASE}/)'),
            (r'^scripts\/', 'Hardcoded scripts/ reference (should be {SCRIPTS_BASE}/)'),
            (r'["\']scripts\/', 'Hardcoded scripts/ reference (should be {SCRIPTS_BASE}/)'),
            (r'python scripts\/', 'Direct python scripts/ call (should use {SCRIPTS_BASE}/)'),
            
            # Templates references
            (r'(?<!\{TEMPLATES_BASE\})\/templates\/', 'Hardcoded templates/ reference (should be {TEMPLATES_BASE}/)'),
            (r'^templates\/', 'Hardcoded templates/ reference (should be {TEMPLATES_BASE}/)'),
            (r'["\']templates\/', 'Hardcoded templates/ reference (should be {TEMPLATES_BASE}/)'),
            
            # Data outputs references  
            (r'(?<!\{DATA_OUTPUTS\})\/data\/outputs\/', 'Hardcoded data/outputs/ reference (should be {DATA_OUTPUTS}/)'),
            (r'^data\/outputs\/', 'Hardcoded data/outputs/ reference (should be {DATA_OUTPUTS}/)'),
            (r'["\']data\/outputs\/', 'Hardcoded data/outputs/ reference (should be {DATA_OUTPUTS}/)'),
        ]
