...
class CLIServiceWrapper:
    ...
    def __init__(self, service_name: str, scripts_dir: Optional[Path] = None):
        ...
        self.logger.info(f"CLI wrapper initialized for {service_name} - Global: {self.global_available}, Local: {self.local_available}")
    ...
    def execute_command(self, command: str, *args, **kwargs) -> Tuple[bool, str, str]:
        ...
        if not self.is_available():
            raise CLIServiceNotFoundError(f"CLI service '{self.service_name}' is not available")
        ...
        if self.global_available:
            try:
                ...
                self.logger.warning(f"Global command failed: {e}, falling back to local execution")
        ...
        raise CLIServiceNotFoundError(f"No execution method available for {self.service_name}")
    ...
    def _execute_global_command(self, args: List[str]) -> Tuple[bool, str, str]:
        ...
        try:
            ...
            raise CLIExecutionError(f"Command failed with return code {e.returncode}: {e.stderr}")
    ...
    def _execute_local_command(self, args: List[str]) -> Tuple[bool, str, str]:
        ...
        try:
            ...
            raise CLIExecutionError(f"Local command failed with return code {e.returncode}: {e.stderr}")
    ...
    def get_service_info(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "execution_mode": "global" if self.global_available else "local" if self.local_available else "unavailable"
        }
...
def execute_cli_command(service_name: str, command: str, *args, **kwargs) -> Tuple[bool, str, str]:
    """
