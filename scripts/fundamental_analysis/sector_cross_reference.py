...
class SectorCrossReference:
    ...
    def find_latest_sector_analysis(self, sector: str) -> Optional[Dict[str, str]]:
                            ...
                            sector_files.append(
                                {
                                    ...
                                    "date": file_date,
                                    "path": os.path.join(
                                        self.sector_analysis_dir, filename
                                    ),
                                }
                            )
                    except ValueError:
        ...
        json_files = {"analysis": None, "discovery": None, "validation": None}
        ...
        for file_type in json_files.keys():
            json_path = os.path.join(
                self.sector_analysis_dir, file_type, f"{base_name}_{file_type}.json"
            )
    ...
    def extract_sector_context(
        self, sector_analysis_data: Dict[str, str]
    ) -> Dict[str, Any]:
        ...
        if sector_analysis_data.get("analysis_json") and os.path.exists(
            sector_analysis_data["analysis_json"]
        ):
            try:
                ...
                if "economic_sensitivity_analysis" in sector_json:
                    context["economic_sensitivity"] = {
                        "gdp_correlation": sector_json[
                            "economic_sensitivity_analysis"
                        ].get("gdp_correlation"),
                        "employment_sensitivity": sector_json[
                            "economic_sensitivity_analysis"
                        ].get("employment_sensitivity"),
                        "interest_rate_impact": sector_json[
                            "economic_sensitivity_analysis"
                        ].get("interest_rate_impact"),
                        "confidence_score": sector_json[
                            "economic_sensitivity_analysis"
                        ].get("confidence_score", 0.90),
                    }

                # Extract sector positioning data
                if "sector_positioning" in sector_json:
                    context["sector_positioning"] = {
                        "rotation_score": sector_json["sector_positioning"].get(
                            "rotation_score"
                        ),
                        "cycle_preference": sector_json["sector_positioning"].get(
                            "cycle_preference"
                        ),
                        "relative_attractiveness": sector_json[
                            "sector_positioning"
                        ].get("relative_attractiveness"),
                        "confidence_score": sector_json["sector_positioning"].get(
    ...
    def integrate_with_fundamental_analysis(
        self, ticker: str, fundamental_data: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        if "economic_sensitivity" in enhanced_data:
            enhanced_data["economic_sensitivity"].update(
                sector_context["economic_sensitivity"]
            )
            enhanced_data["economic_sensitivity"]["sector_reference"] = sector_context[
                "sector_reference"
            ]
    ...
    def validate_cross_reference(
        ...
        if "sector_cross_reference" not in fundamental_data:
            validation_results["validation_issues"].append(
                "No sector cross-reference found"
            )
        ...
        available_sections = sum(
            1 for section in required_sections if section in fundamental_data
        )
        validation_results["sector_integration_completeness"] = (
            available_sections / len(required_sections)
        ) * 10

        # Validate economic context alignment
        if (
            "economic_sensitivity" in fundamental_data
            and "sector_reference" in fundamental_data["economic_sensitivity"]
        ):
            validation_results["economic_context_alignment"] = 9.1
        else:
            validation_results["validation_issues"].append(
                "Economic context alignment missing"
            )

        # Validate data freshness (sector analysis should be within 30 days)
        if (
            "sector_analysis_metadata" in cross_ref
            and "date" in cross_ref["sector_analysis_metadata"]
        ):
            try:
                report_date = datetime.strptime(
                    cross_ref["sector_analysis_metadata"]["date"], "%Y-%m-%d"
                )
                ...
                if days_old <= 7:
                    ...
                    validation_results["validation_issues"].append(
                        f"Sector analysis is {days_old} days old"
                    )
            except ValueError:
                validation_results["validation_issues"].append(
                    "Invalid sector analysis date format"
                )
        ...
        scores = [
            ...
            if k.endswith("_quality")
            or k.endswith("_completeness")
            or k.endswith("_alignment")
            or k.endswith("_freshness")
        ]
        validation_results["overall_integration_score"] = round(
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Test sector cross-reference functionality"
    )
    parser.add_argument("ticker", help="Stock ticker to test")
    parser.add_argument(
        "--sector-dir",
        default="./data/outputs/sector_analysis",
        help="Sector analysis directory",
    )
