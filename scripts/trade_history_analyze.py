...
class TradeHistoryAnalysis:
    ...
    def load_discovery_data(self) -> Dict[str, Any]:
        ...
        required_keys = ['portfolio_summary', 'ticker_performance', 'performance_metrics']
        for key in required_keys:
            if key not in discovery_data:
                raise ValueError(f"Missing required key '{key}' in discovery data")

        logger.info(f"Discovery data loaded successfully: {discovery_data['portfolio_summary']['total_trades']} total trades")
        return discovery_data

    def extract_trade_data(self, discovery_data: Dict[str, Any]) -> Tuple[List[Dict[str, Any]], List[Dict[str, Any]]]:
        """
        ...
        for _, row in df.iterrows():
            ...
            if status == 'Closed':
                ...
                if trade.get('PnL') is not None and trade.get('Return') is not None:
                    ...
                    logger.warning(f"Closed trade {trade.get('Position_UUID')} missing PnL or Return data")
            elif status in ['Open', 'Active']:
                active_trades.append(trade)

        logger.info(f"Trade data extracted: {len(closed_trades)} closed, {len(active_trades)} active")
        return closed_trades, active_trades

    def analyze_signal_effectiveness(self, closed_trades: List[Dict[str, Any]]) -> Dict[str, Any]:
        """
        ...
        if not closed_trades:
            logger.warning("No closed trades available for signal effectiveness analysis")
            return {
        ...
        for strategy, trades in strategy_groups.items():
            ...
            loss_returns = [t.get('Return') for t in losses if t.get('Return') is not None]
        ...
        valid_mfe_trades = [t for t in closed_trades if t.get('Max_Favourable_Excursion') is not None]
        valid_exit_eff_trades = [t for t in closed_trades if t.get('Exit_Efficiency') is not None]
        valid_mfe_ratio_trades = [t for t in closed_trades if t.get('MFE_MAE_Ratio') is not None]
        
        exit_analysis = {}
        if valid_exit_eff_trades:
            # Calculate exit efficiency safely
            exit_eff_values = [t.get('Exit_Efficiency') for t in valid_exit_eff_trades if t.get('Exit_Efficiency') is not None]
            mfe_ratio_values = [t.get('MFE_MAE_Ratio') for t in valid_mfe_ratio_trades if t.get('MFE_MAE_Ratio') is not None]
            
            exit_analysis = {
                "exit_efficiency_metrics": {
                    "overall_exit_efficiency": np.mean(exit_eff_values) if exit_eff_values else 0,
                    "mfe_capture_rate": np.mean(mfe_ratio_values) if mfe_ratio_values else 0,
        ...
        return {
            "entry_signal_analysis": {
                ...
                "total_strategies_analyzed": len([s for s in entry_analysis.values() if isinstance(s, dict) and s.get('analysis_possible', True)]),
                "strategies_excluded": len([s for s in entry_analysis.values() if isinstance(s, dict) and not s.get('analysis_possible', True)])
            },
            "exit_signal_analysis": exit_analysis
        }

    def perform_statistical_analysis(self, closed_trades: List[Dict[str, Any]]) -> Dict[str, Any]:
        """
        ...
        returns = [t.get('Return', 0) for t in closed_trades if t.get('Return') is not None]
        ...
        return_analysis = {
            "return_distribution": {
                ...
                "normality_test_p_value": float(normality_p_value) if normality_p_value else None,
        ...
        if len(returns) >= 10 and np.std(returns_array) > 0:
            ...
            downside_deviation = np.std(downside_returns, ddof=1) if len(downside_returns) > 1 else np.std(returns_array, ddof=1)
    ...
    def analyze_patterns_and_quality(self, closed_trades: List[Dict[str, Any]]) -> Dict[str, Any]:
        """
        ...
        for quality, trades in quality_distribution.items():
            if trades:
                returns = [t.get('Return', 0) for t in trades if t.get('Return') is not None]
                quality_analysis[quality] = {
                    "count": len(trades),
                    "percentage": len(trades) / len(closed_trades),
                    "avg_return": np.mean(returns) if returns else 0,
                    "characteristics": self._derive_quality_characteristics(quality, trades)
                }
        ...
        if len(closed_trades) >= 10:
            ...
            if trades_with_dates:
                ...
                for month in range(1, 13):
                    month_trades = [t for t in trades_with_dates if t.get('entry_month') == month]
                    if month_trades:
                        month_returns = [t.get('Return', 0) for t in month_trades if t.get('Return') is not None]
                        monthly_performance[f"month_{month}"] = {
                            "trade_count": len(month_trades),
                            "win_rate": len([t for t in month_trades if t.get('Return', 0) > 0]) / len(month_trades),
                            "avg_return": np.mean(month_returns) if month_returns else 0
                        }
    ...
    def _derive_quality_characteristics(self, quality: str, trades: List[Dict[str, Any]]) -> List[str]:
        ...
        duration_values = [t.get('Duration_Days') for t in trades if t.get('Duration_Days') is not None]
    ...
    def generate_optimization_opportunities(self, analysis_results: Dict[str, Any]) -> Dict[str, Any]:
        """
    ...
    def assess_risk_and_portfolio(self, closed_trades: List[Dict[str, Any]], active_trades: List[Dict[str, Any]]) -> Dict[str, Any]:
        """
        ...
        if closed_trades:
            ...
            if returns:
                ...
                risk_assessment["drawdown_analysis"] = {
                    ...
                    "downside_deviation": float(np.std(returns_array[returns_array < 0], ddof=1)) if any(returns_array < 0) else 0,
                    "confidence": min(0.9, 0.4 + (len(returns) / 20))
                }
        ...
        if active_trades:
            ...
            sector_concentration = {sector: count/total_active for sector, count in sectors.items()}

            risk_assessment["portfolio_risk_metrics"] = {
                "active_positions": total_active,
                "sector_concentration": sector_concentration,
                "position_diversification": len(sectors) / total_active if total_active > 0 else 0,
                "confidence": 0.8
            }
    ...
    def _map_ticker_to_sector(self, ticker: str) -> str:
        ...
        tech_tickers = ['AAPL', 'MSFT', 'GOOGL', 'AMZN', 'META', 'NVDA', 'TSLA', 'AMD', 'SMCI']
    ...
    def calculate_comprehensive_confidence(self, analysis_results: Dict[str, Any]) -> Dict[str, float]:
        """
        ...
        if strategies_analyzed > 0:
            confidence_scores["signal_effectiveness"] = min(0.9, 0.6 + (strategies_analyzed * 0.2))
        else:
    ...
    def generate_analysis_output(self, analysis_results: Dict[str, Any], confidence_scores: Dict[str, float]) -> Dict[str, Any]:
        """
        ...
        analysis_output = {
            ...
            "analysis_metadata": {
                ...
                "signal_effectiveness_confidence": confidence_scores["signal_effectiveness"]
            },
            "sample_validation": {
            ...
            "performance_measurement": analysis_results.get('performance_measurement', {}),
            ...
            "next_phase_inputs": {
                ...
                "analysis_package_path": str(self.output_dir / f"{self.portfolio_name}_{self.execution_date.strftime('%Y%m%d')}.json"),
    ...
    def _generate_analysis_limitations(self) -> List[str]:
        ...
        if len(self.closed_trades) < 25:
            limitations.append(f"Sample size ({len(self.closed_trades)} closed trades) below recommended minimum of 25 for robust statistical analysis")
        
        if len(self.closed_trades) < 15:
            limitations.append("Limited statistical power due to small sample size - results should be interpreted cautiously")
        ...
        insufficient_strategies = [s for s, count in strategy_counts.items() if count < 5]
        if insufficient_strategies:
            limitations.append(f"Strategies with insufficient samples excluded from analysis: {', '.join(insufficient_strategies)}")
    ...
    def _extract_critical_findings(self, analysis_results: Dict[str, Any]) -> List[str]:
        ...
        if return_test.get('significant_at_95', False):
            findings.append("Strategy returns are statistically significant at 95% confidence level")
        else:
            findings.append("Strategy returns lack statistical significance - require larger sample or improved signals")
        ...
        if win_rate > 0.6:
            findings.append(f"Strong win rate of {win_rate:.1%} indicates effective signal quality")
        elif win_rate < 0.5:
            findings.append(f"Below-average win rate of {win_rate:.1%} suggests signal refinement needed")
            
        # Sample size adequacy
        if len(self.closed_trades) < 15:
            findings.append("Critical: Insufficient sample size for reliable statistical conclusions")
    ...
    def execute_analysis(self) -> Dict[str, Any]:
        ...
        """
        logger.info(f"Starting trade history analysis for portfolio: {self.portfolio_name}")
        ...
        try:
            ...
            self.closed_trades, self.active_trades = self.extract_trade_data(self.discovery_data)
            ...
            analysis_results['signal_effectiveness'] = self.analyze_signal_effectiveness(self.closed_trades)
            
            # Statistical performance measurement  
            analysis_results['statistical_analysis'] = self.perform_statistical_analysis(self.closed_trades)
            
            # Pattern recognition and quality classification
            analysis_results['pattern_recognition'] = self.analyze_patterns_and_quality(self.closed_trades)
            
            # Optimization opportunities
            analysis_results['optimization_opportunities'] = self.generate_optimization_opportunities(analysis_results)
            
            # Risk assessment
            analysis_results['risk_assessment'] = self.assess_risk_and_portfolio(self.closed_trades, self.active_trades)
            
            # Step 5: Calculate comprehensive confidence scores
            confidence_scores = self.calculate_comprehensive_confidence(analysis_results)
            
            # Step 6: Generate comprehensive analysis output
            analysis_output = self.generate_analysis_output(analysis_results, confidence_scores)
