...
from errors import (
    TwitterSystemError, ValidationError, TemplateError, DataError,
    ConfigurationError, ProcessingError, TypeValidationError,
    validation_failed, template_not_found, data_file_not_found,
    invalid_data_format, missing_required_field
)
from error_handler import ErrorHandler, fail_if_missing_file, fail_if_invalid_data, fail_if_below_threshold
...
class TestErrorHandler:
    ...
    def test_file_format_validation(self):
        ...
        with tempfile.NamedTemporaryFile(suffix=".txt", delete=False) as temp_file:
            ...
            # Test correct format
            result = self.error_handler.validate_file_format(
                temp_path,
                ".txt"
            )
    ...
    def test_graceful_degradation(self):
        ...
        try:
            error_handler.handle_validation_error(
                "fundamental",
                {"overall_score": 7.5, "issues": ["minor issue"], "failed_criteria": ["minor"]},
                fail_fast=False
            )
        except:
            pass
