...
class ContractComplianceMonitor:
    ...
    def _generate_system_report(self, contract_reports: List[ContractComplianceReport], 
                               discovery_result: ContractDiscoveryResult) -> SystemComplianceReport:
        """Generate overall system compliance report"""
        
        system_report = SystemComplianceReport(
            report_timestamp=datetime.now(),
            total_contracts=len(contract_reports)
        )
    ...
    def export_report(self, report: SystemComplianceReport, output_file: Path) -> ProcessingResult:
        ...
        try:
            ...
            return ProcessingResult(
                success=True,
                operation="export_compliance_report"
            )
    ...
    def print_summary_report(self, report: SystemComplianceReport):
        ...
        print(f"   ✅ Healthy: {report.healthy_contracts} ({report.healthy_contracts/report.total_contracts*100:.1f}%)")
        print(f"   ⚠️  Warning: {report.warning_contracts} ({report.warning_contracts/report.total_contracts*100:.1f}%)")
        print(f"   🚨 Critical: {report.critical_contracts} ({report.critical_contracts/report.total_contracts*100:.1f}%)")
        print(f"   ❌ Missing: {report.missing_contracts} ({report.missing_contracts/report.total_contracts*100:.1f}%)")
        print()
