...
from scripts.utils.plotly_theme_mapper import PlotlyThemeMapper, MultiFormatExporter
...
class Phase4TestSuite:
    ...
    def __init__(self):
        ...
        # Create components
        self.layout_manager = create_plotly_layout_manager(layout_config, self.theme_manager)
    ...
    def test_plotly_layout_manager(self) -> Dict[str, Any]:
        ...
        try:
            ...
            assert fig.layout.width == 1600, f"Expected width 1600, got {fig.layout.width}"
            assert fig.layout.height == 1200, f"Expected height 1200, got {fig.layout.height}"
            ...
            chart_fig = go.Figure()
            chart_result = self.plotly_generator.create_enhanced_monthly_bars(chart_fig, monthly_data, "light")
            
            if chart_result and hasattr(chart_result, 'data'):
                fig = self.layout_manager.add_chart_to_subplot(fig, list(chart_result.data), 2, 1, "Monthly Performance")
            
            # Test theme application
            fig = self.layout_manager.apply_dashboard_theme(fig, "Test Dashboard", "Phase 4 Validation", "light")
            ...
            print(f"  ✅ Layout manager: {fig.layout.width}x{fig.layout.height}, {len(fig.data)} traces")
    ...
    def test_enhanced_theme_system(self) -> Dict[str, Any]:
        ...
        try:
            ...
            expected_templates = ["sensylate_light", "sensylate_dark", "sensylate_light_hd", 
                                "sensylate_dark_hd", "sensylate_dashboard"]
            
            for template_name in expected_templates:
                assert template_name in pio.templates, f"Template {template_name} not found"
            ...
            for mode, high_dpi, dashboard in template_tests:
                ...
                themed_fig = theme_mapper.apply_template(test_fig, mode, high_dpi, dashboard)
                
                template_name = theme_mapper.get_template_name(mode, high_dpi, dashboard)
            ...
            print(f"  ✅ Theme system: {len(expected_templates)} templates, font integration working")
    ...
    def test_high_dpi_export_configuration(self) -> Dict[str, Any]:
        ...
        try:
            ...
            assert export_config["width"] == 1600, "Export width not configured correctly"
            assert export_config["height"] == 1200, "Export height not configured correctly"
            assert export_config["scale"] == 3.0, "Export scale not configured correctly"
            assert export_config["format"] == "png", "Export format not configured correctly"
            ...
            layout_export_config = self.layout_manager.configure_high_dpi_export(scale=2.0)
            
            assert "width" in layout_export_config, "Layout export config missing width"
            assert "height" in layout_export_config, "Layout export config missing height"
    ...
    def test_multi_format_export_system(self) -> Dict[str, Any]:
        ...
        try:
            ...
            results["multi_format_export"] = {
                ...
                "total_formats": len([f for f in format_tests.values() if f["supported"]])
            }
            
            supported_count = len([f for f in format_tests.values() if f["supported"]])
            print(f"  ✅ Multi-format export: {supported_count}/{len(supported_formats)} formats supported")
    ...
    def test_layout_optimization(self) -> Dict[str, Any]:
        ...
        try:
            ...
            for chart_type in chart_types:
                ...
                # Apply optimization
                optimized_fig = self.layout_manager.optimize_chart_layout(fig, chart_type, 2, 1)
            ...
            assert isinstance(responsive_config, dict), "Responsive config not a dictionary"
            
            expected_breakpoints = ["mobile", "tablet", "desktop", "large"]
            for breakpoint in expected_breakpoints:
                assert breakpoint in responsive_config, f"Missing breakpoint: {breakpoint}"
                assert "width" in responsive_config[breakpoint], f"Missing width for {breakpoint}"
                assert "height" in responsive_config[breakpoint], f"Missing height for {breakpoint}"
            ...
            print(f"  ✅ Layout optimization: {len(chart_types)} chart types, {len(responsive_config)} breakpoints")
    ...
    def test_integration_with_existing_charts(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if monthly_result:
                ...
                themed_fig = self.theme_mapper.apply_font_configuration(monthly_result, "light")
                enhanced_fig = self.theme_mapper.apply_template(themed_fig, "light", high_dpi=True)
            ...
            fig = go.Figure()
            waterfall_result = self.plotly_generator.create_waterfall_chart(fig, trade_data[:20], "light")  # Limit for testing
            if waterfall_result:
                # Test export configuration
                export_config = self.plotly_generator.configure_export_settings(format="png", scale=2.0)
            ...
            if scatter_result:
                ...
                enhanced_scatter = self.theme_mapper.apply_font_configuration(scatter_result, "light")
                dashboard_scatter = self.theme_mapper.apply_template(enhanced_scatter, "light", dashboard=True)
            ...
            successful_integrations = len([test for test in chart_tests.values() 
            ...
            print(f"  ✅ Integration: {successful_integrations}/{len(chart_tests)} chart types integrated")
    ...
    def run_comprehensive_tests(self) -> Dict[str, Any]:
        ...
        print(f"🎯 Status: {'✅ PHASE 4 COMPLETE' if pass_rate >= 80 else '❌ NEEDS ATTENTION'}")
