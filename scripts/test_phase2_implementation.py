...
def test_theme_mapper():
    ...
    try:
        ...
        if "template" in layout_config and layout_config["template"] == "sensylate_light":
...
def test_monthly_bars_implementation():
    ...
    try:
        ...
        fig = go.Figure()
        result = plotly_gen.create_enhanced_monthly_bars(fig, test_data["monthly_data"], "light")
...
def test_donut_chart_implementation():
    ...
    try:
        ...
        import plotly.graph_objects as go
        fig = go.Figure()
        result = plotly_gen.create_enhanced_donut_chart(fig, test_data["quality_data"], "light")
...
def test_json_schema_export():
    ...
    try:
        ...
        monthly_schema = plotly_gen.export_chart_config("enhanced_monthly_bars", {"mode": "light"})
        
        if all(key in monthly_schema for key in ["engine", "chart_type", "version", "theme", "data_requirements"]):
        ...
        donut_schema = plotly_gen.export_chart_config("enhanced_donut_chart", {"mode": "dark"})
...
def test_visual_comparison():
    ...
    try:
        ...
        if Path(monthly_result["comparison_path"]).exists():
            ...
            print(f"    - Similarity: {monthly_result['similarity_metrics']['similarity_percent']:.1f}%")
        else:
        ...
        if Path(donut_result["comparison_path"]).exists():
            ...
            print(f"    - Similarity: {donut_result['similarity_metrics']['similarity_percent']:.1f}%")
        else:
            print("  ✗ Donut chart comparison failed")
            return False
        
        # Generate comparison report
        report_path = comparison_framework.generate_test_report([monthly_result, donut_result])
