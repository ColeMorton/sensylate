...
class TradingCalculationEngine:
    ...
    def _validate_all_trades(self):
        ...
        if validation_errors:
            raise ValidationError(f"Trade validation failed:\n" + "\n".join(validation_errors))
    ...
    def calculate_portfolio_performance(self) -> Dict[str, Any]:
        ...
        # Risk-reward metrics
        profit_factor = abs(winning_pnl / losing_pnl) if abs(losing_pnl) > FINANCIAL_TOLERANCES["pnl_accuracy"] else float('inf')
        avg_win_loss_ratio = abs(avg_win / avg_loss) if abs(avg_loss) > FINANCIAL_TOLERANCES["pnl_accuracy"] else float('inf')
        ...
        try:
            ...
            if len(returns) > 1:
                ...
                validation_results["sharpe_ratio_validation"] = {
                    ...
                    "validation_confidence": 0.95 if sharpe_variance <= FINANCIAL_TOLERANCES["sharpe_ratio"] else 0.65
                }
    ...
    def get_discovery_data(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "performance_metrics": {
                ...
                "average_loss_return": metrics["avg_loss"] / abs(metrics["avg_loss"]) * abs(metrics["avg_return"]) if metrics["losing_trades"] > 0 else 0.0,
