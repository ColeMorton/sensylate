#!/usr/bin/env python3
"""
Test script to validate fundamental analysis discovery JSON files against the schema.
"""
...
def load_schema():
    """Load the discovery schema from the schemas directory."""
    schema_path = Path(__file__).parent / "schemas" / "fundamental_analysis_discovery_schema.json"
...
def validate_discovery_file(discovery_data, schema, file_path):
    """Validate a discovery file against the schema."""
    try:
...
def main():
    ...
    print("🔍 Testing Fundamental Analysis Discovery Schema Validation")
    print("=" * 60)
    ...
    discovery_dir = Path(__file__).parent.parent / "data" / "outputs" / "fundamental_analysis" / "discovery"
    test_files = [
        "VTRS_20250725_discovery.json",  # Most recent
        "FANG_20250724_discovery.json", 
        "PKG_20250724_discovery.json",
        "TFC_20250724_discovery.json",
        "ADBE_20250723_discovery.json",
        "CMA_20250723_discovery.json",
        "TEAM_20250723_discovery.json"
    ]
    
    validation_results = []
    
    print(f"\n📁 Testing {len(test_files)} discovery files:")
    print("-" * 40)
