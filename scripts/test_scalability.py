...
def generate_synthetic_trades(count: int) -> List[TradeData]:
    ...
    tickers = ['AAPL', 'MSFT', 'GOOGL', 'AMZN', 'TSLA', 'META', 'NVDA', 'AMD', 
               'NFLX', 'CRM', 'ADBE', 'INTC', 'CSCO', 'ORCL', 'IBM', 'PYPL',
               'SHOP', 'SQ', 'UBER', 'LYFT', 'SNOW', 'PLTR', 'CRWD', 'ZM']
...
def test_label_optimization():
    ...
    for month_count, expected_category in [(3, 'compact'), (6, 'medium'), (12, 'condensed')]:
        monthly_data = generate_synthetic_monthly_data(month_count)
        ...
        print(f"   Labels: {optimized_labels[:3]}..." if len(optimized_labels) > 3 else f"   Labels: {optimized_labels}")
        print()
