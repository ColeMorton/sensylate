...
from errors import ValidationError, TypeValidationError
    ...
    def _validate_criteria(self) -> None:
        ...
        if not 0.8 <= total_weight <= 1.2:
            raise ValidationError(
                ...
                context={"total_weight": total_weight, "criteria_count": len(self.criteria)}
            )
    
    @abstractmethod
    def calculate_score(self, data: Dict[str, Any], context: Optional[Dict[str, Any]] = None) -> ScoringResult:
...
class WeightedScoringAlgorithm(BaseScoringAlgorithm):
        ...
        for criterion in self.criteria:
            try:
                ...
                self.logger.log_error(
                    e,
                    {"criterion": criterion.name, "data_keys": list(data.keys())}
                )
    ...
    def _evaluate_criterion(self, data: Dict[str, Any], criterion: ScoringCriteria, context: Optional[Dict[str, Any]]) -> float:
...
class TemplateScoringEngine:
    ...
    def _initialize_scoring_algorithms(self) -> Dict[str, Dict[str, BaseScoringAlgorithm]]:
    ...
    def score_template(self, content_type: str, template_variant: str, data: Dict[str, Any], context: Optional[Dict[str, Any]] = None) -> ScoringResult:
    ...
    def score_all_templates(self, content_type: str, data: Dict[str, Any], context: Optional[Dict[str, Any]] = None) -> Dict[str, ScoringResult]:
    ...
    def update_scoring_criteria(self, content_type: str, template_variant: str, criteria: List[ScoringCriteria]) -> None:
