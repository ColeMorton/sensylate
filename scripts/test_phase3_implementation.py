...
class Phase3TestSuite:
    ...
    def __init__(self):
        ...
        scalability_config = {
            "scalability": {
                ...
                "scatter_density_thresholds": {"low": 50, "medium": 150, "high": 200},
                "clustering": {"enabled": True, "min_samples": 3, "eps": 0.5},
                "compression": {"enabled": True, "max_labels": 20},
            }
        }

        self.scalability_manager = create_scalability_manager(scalability_config)
        self.comparison_framework = ChartComparisonFramework(
            "data/outputs/phase3_tests"
        )
    ...
    def test_waterfall_chart_implementation(self) -> Dict[str, Any]:
        ...
        for size in [
            "small",
            "medium",
        ]:  # Skip large to avoid performance bands in basic test
            ...
            try:
                ...
                result = self.plotly_generator.create_waterfall_chart(
                    fig, trades, "light"
                )
                end_time = time.time()
                ...
                assert (
                    result is not None
                ), f"Waterfall chart returned None for {size} dataset"
                assert isinstance(
                    result, go.Figure
                ), f"Expected Figure, got {type(result)}"
                assert (
                    len(result.data) > 0
                ), f"No traces added to waterfall chart for {size} dataset"

                # Check for cumulative line trace
                has_line = any(trace.type == "scatter" for trace in result.data)
                assert (
                    has_line
                ), f"Missing cumulative line in waterfall chart for {size} dataset"

                # Performance validation
                generation_time = end_time - start_time
                assert (
                    generation_time < 2.0
                ), f"Waterfall chart generation too slow: {generation_time:.2f}s"
                ...
                print(
                    f"  ✅ {size.title()} dataset: {len(trades)} trades, {len(result.data)} traces, {generation_time:.3f}s"
                )
    ...
    def test_enhanced_scatter_implementation(self) -> Dict[str, Any]:
        ...
        for size in ["small", "medium", "large"]:
            ...
            try:
                ...
                fig = go.Figure()
                start_time = time.time()
                result = self.plotly_generator.create_enhanced_scatter(
                    fig, trades, "light"
                )
                end_time = time.time()
                ...
                assert (
                    result is not None
                ), f"Scatter plot returned None for {size} dataset"
                assert isinstance(
                    result, go.Figure
                ), f"Expected Figure, got {type(result)}"
                assert (
                    len(result.data) > 0
                ), f"No traces added to scatter plot for {size} dataset"

                # Check for trend line trace (should exist for normal scatter)
                if size != "large":  # Large datasets might use clustering
                    has_trend = any(
                        trace.mode and "lines" in trace.mode for trace in result.data
                    )
                    assert (
                        has_trend
                    ), f"Missing trend line in scatter plot for {size} dataset"

                # Performance validation
                generation_time = end_time - start_time
                assert (
                    generation_time < 3.0
                ), f"Scatter plot generation too slow: {generation_time:.2f}s"
    ...
    def test_scalability_integration(self) -> Dict[str, Any]:
        ...
        try:
            ...
            small_category = self.scalability_manager.detect_trade_volume_category(
                small_trades
            )
            medium_category = self.scalability_manager.detect_trade_volume_category(
                medium_trades
            )
            large_category = self.scalability_manager.detect_trade_volume_category(
                large_trades
            )

            assert (
                small_category == "small"
            ), f"Expected 'small', got '{small_category}'"
            assert medium_category in [
                "medium",
                "small",
            ], f"Expected 'medium'/'small', got '{medium_category}'"
            assert large_category in [
                "large",
                "medium",
            ], f"Expected 'large'/'medium', got '{large_category}'"

            # Test scatter density detection
            scatter_category = self.scalability_manager.detect_scatter_density_category(
                large_trades
            )
            assert scatter_category in [
                "high",
                "medium",
                "low",
            ], f"Invalid scatter category: {scatter_category}"
            ...
            print(
                f"  ✅ Volume detection: {len(small_trades)}→{small_category}, {len(medium_trades)}→{medium_category}, {len(large_trades)}→{large_category}"
            )
            print(f"  ✅ Scatter density: {scatter_category}")
            print(f"  ✅ Performance bands: {len(bands)} categories")

        except Exception as e:
            results["scalability_detection"] = {"status": f"❌ FAIL: {str(e)}"}
    ...
    def test_performance_bands_chart(self) -> Dict[str, Any]:
        ...
        try:
            ...
            assert (
                generation_time < 2.0
            ), f"Performance bands generation too slow: {generation_time:.2f}s"
            ...
            print(
                f"  ✅ Performance bands: {len(large_trades)} trades → {len(result.data)} traces, {generation_time:.3f}s"
            )

        except Exception as e:
            results["performance_bands"] = {"status": f"❌ FAIL: {str(e)}"}
    ...
    def test_clustering_functionality(self) -> Dict[str, Any]:
        ...
        try:
            ...
            total_points = (
                cluster_info["clustered_points"] + cluster_info["noise_points"]
            )
            assert total_points == len(
                large_trades
            ), f"Point count mismatch: {total_points} != {len(large_trades)}"

            # Test clustered scatter chart creation
            fig = go.Figure()
            start_time = time.time()
            result = self.plotly_generator._create_clustered_scatter(
                fig, cluster_info, "light"
            )
            end_time = time.time()
            ...
            assert (
                generation_time < 1.0
            ), f"Clustering visualization too slow: {generation_time:.2f}s"
            ...
            print(
                f"  ✅ Clustering: {len(large_trades)} trades → {cluster_info['total_clusters']} clusters"
            )
            print(
                f"  ✅ Grouped: {cluster_info['clustered_points']}, Individual: {cluster_info['noise_points']}"
            )
            print(
                f"  ✅ Visualization: {len(result.data)} traces, {generation_time:.3f}s"
            )

        except Exception as e:
            results["clustering"] = {"status": f"❌ FAIL: {str(e)}"}
    ...
    def test_advanced_styling_system(self) -> Dict[str, Any]:
        ...
        try:
            ...
            for chart_type in chart_types:
                ...
                styling_config = (
                    self.plotly_generator.theme_mapper.get_advanced_styling_config(
                        chart_type, "light"
                    )
                )
                ...
                assert isinstance(
                    styling_config, dict
                ), f"Styling config should be dict for {chart_type}"
                ...
                for volume in ["small", "medium", "large"]:
                    scalability_config = (
                        self.plotly_generator.theme_mapper.get_scalability_styling(
                            volume, "light"
                        )
                    )
                    assert isinstance(
                        scalability_config, dict
                    ), f"Scalability config should be dict for {volume}"
                    assert (
                        "opacity" in scalability_config
                    ), f"Missing opacity in {volume} config"
                    assert (
                        "detail_level" in scalability_config
                    ), f"Missing detail_level in {volume} config"

            # Test theme application
            fig = go.Figure()
            self.plotly_generator.theme_mapper.apply_advanced_styling(
                fig, "scatter", "light", title="Test Chart"
            )

            # Validate figure has been styled
            assert (
                fig.layout.template is not None or len(str(fig.layout)) > 100
            ), "Figure not properly styled"
            ...
            results["advanced_styling"] = {"status": f"❌ FAIL: {str(e)}"}
    ...
    def test_json_schema_export(self) -> Dict[str, Any]:
        ...
        try:
            ...
            for chart_type in chart_types:
                # Export schema
                schema = self.plotly_generator.export_chart_config(
                    chart_type, {"mode": "light"}
                )

                # Validate schema structure
                assert isinstance(
                    schema, dict
                ), f"Schema should be dict for {chart_type}"
                assert "engine" in schema, f"Missing 'engine' in {chart_type} schema"
                assert (
                    "chart_type" in schema
                ), f"Missing 'chart_type' in {chart_type} schema"
                ...
                assert (
                    "chart_config" in schema
                ), f"Missing 'chart_config' in {chart_type} schema"
                assert (
                    "data_requirements" in schema
                ), f"Missing 'data_requirements' in {chart_type} schema"

                # Validate data requirements
                data_req = schema["data_requirements"]
                assert (
                    "fields" in data_req
                ), f"Missing 'fields' in {chart_type} data requirements"
                assert (
                    "format" in data_req
                ), f"Missing 'format' in {chart_type} data requirements"
                assert (
                    len(data_req["fields"]) > 0
                ), f"Empty fields in {chart_type} data requirements"
                ...
                schema_path = (
                    Path("data/outputs/phase3_tests")
                    / f"{chart_type}_schema_light.json"
                )
            ...
            results["json_schema"] = {"status": f"❌ FAIL: {str(e)}"}
    ...
    def run_comprehensive_tests(self) -> Dict[str, Any]:
        ...
        print(
            f"🎯 Status: {'✅ PHASE 3 COMPLETE' if pass_rate >= 80 else '❌ NEEDS ATTENTION'}"
        )
