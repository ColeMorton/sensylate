...
class ValidationEngine(ComponentLifecycle):
    ...
    def _do_configure(self) -> None:
        ...
        self.logger.info(
            "Configuring ValidationEngine - performing expensive validation operations..."
        )
        ...
        self.logger.info(
            "ValidationEngine configuration completed - all expensive operations cached"
        )
    ...
    def _cache_service_validation(self) -> None:
        ...
        self.logger.debug(
            f"Service validation cached in {duration.total_seconds():.2f}s"
        )
    ...
    def _cache_schema_validation(self) -> None:
        ...
        if self._contract_discovery_cache is None:
            self.logger.warning(
                "Cannot cache schema validation - no contracts discovered"
            )
            return
    ...
    def validate_service_dependencies(self) -> ProcessingResult:
        ...
        if not self.is_configured():
            raise RuntimeError(
                "ValidationEngine must be configured before validation operations"
            )

        if self._service_validation_cache is None:
            raise RuntimeError(
                "Service validation cache not available - configuration may have failed"
            )
        ...
        try:
            ...
            result = ProcessingResult(
                ...
                error="; ".join(cached_result["validation_results"])
                if cached_result["validation_results"]
                else None,
            )
            ...
            error_msg = (
                f"Failed to retrieve cached service validation results: {str(e)}"
            )
    ...
    def get_validation_cache_info(self) -> Dict[str, Any]:
        ...
        if self._service_validation_cache:
            cache_info["service_validation_cached_at"] = self._service_validation_cache[
                "cached_at"
            ].isoformat()
            cache_info["service_validation_success"] = self._service_validation_cache[
                "success"
            ]
        ...
        if self._schema_validation_cache:
            cache_info["schema_validation_cached_at"] = self._schema_validation_cache[
                "cached_at"
            ].isoformat()
            cache_info["schema_validation_success"] = self._schema_validation_cache[
                "success"
            ]
    ...
    def clear_validation_cache(self) -> None:
        ...
        self.logger.debug(
            f"Clearing validation cache ({len(self._validated_contracts)} CLI contracts)"
        )
