...
from data_contract_discovery import DataContractDiscovery, DataContract, ContractDiscoveryResult
...
class DataPipelineManager:
    ...
    def refresh_all_chart_data(self, skip_errors: bool = False) -> ProcessingResult:
        ...
        try:
            ...
            self.logger.info(f"Processing {len(discovery_result.contracts)} discovered contracts")
    ...
    def _refresh_contracts_for_category(self, category: str, contracts: List[DataContract]) -> ProcessingResult:
        ...
        try:
            ...
            for contract in contracts:
                try:
                    ...
                    if contract_result.success:
                        ...
                        self.logger.warning(f"Failed to fulfill contract {contract.contract_id}: {contract_result.error}")
                except Exception as e:
                    failed_contracts.append(contract.contract_id)
            ...
            result.add_metadata("contract_details", {
                "successful": successful_contracts,
                "failed": failed_contracts
            })
    ...
    def _generate_trade_history_data(self) -> pd.DataFrame:
        ...
        tickers = ["AAPL", "GOOGL", "MSFT", "AMZN", "TSLA", "NVDA", "CRWD", "NFLX", "COST"]
        strategies = ["SMA", "EMA"]
        ...
        for i in range(15):  # Generate 15 sample trades
            ...
            trade = {
                ...
                "Exit_Timestamp": (entry_date + timedelta(days=np.random.randint(1, 120))).strftime("%Y-%m-%d %H:%M:%S"),
        ...
        for category, config in self.chart_data_dependencies.items():
            ...
            for output_file in config["output_files"]:
                if category == "portfolio":
                    ...
                    file_path = self.frontend_data_dir / "portfolio/live-signals" / output_file
                elif category == "trade_history":
                ...
                if file_path.exists():
                    ...
                    if file_status == "stale":
                        category_results["issues"].append(f"{output_file} is {file_age_hours:.1f} hours old")
                else:
                    category_results["files"][output_file] = {
                        "exists": False,
                        "status": "missing"
...
def main():
    ...
    parser.add_argument("--skip-errors", action="store_true", help="Continue despite individual failures")
    ...
    if args.validate_only:
        ...
        if result.success:
            ...
            print(f"   Contracts: {result.metadata.get('successful_contracts', 0)}/{result.metadata.get('total_contracts', 0)}")
            ...
            print(f"   Discovery: {discovery_stats.get('total_discovered', 0)} contracts found in {discovery_stats.get('discovery_time', 0):.2f}s")
...
if __name__ == "__main__":
    # Add numpy import for data generation
    try:
        import numpy as np
    except ImportError:
        print("❌ numpy is required for data generation")
        sys.exit(1)
