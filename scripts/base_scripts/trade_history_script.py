...
class TradeHistoryScript(BaseScript):
    ...
    def execute(
        ...
        **kwargs
        ...
        try:
            # Validate inputs
            self.validate_inputs(analysis_name=analysis_name, date=date, data_path=data_path,
                               template_variant=template_variant, output_path=output_path,
                               validate_content=validate_content, min_win_rate=min_win_rate,
                               min_trades=min_trades, transparency_level=transparency_level)
            ...
            if template_variant:
                ...
                selected_template, template_metadata = self.template_selector.select_optimal_template(
            ...
            if validate_content:
                ...
                overall_score = float(validation_result["overall_assessment"]["overall_reliability_score"].split("/")[0])
            ...
            if validation_result:
                result.validation_score = float(validation_result["overall_assessment"]["overall_reliability_score"].split("/")[0])
            ...
            self.logger.log_error(
                e,
                {"analysis_name": analysis_name, "date": date, "processing_time": processing_time}
            )
    ...
    def _load_trade_data(self, analysis_name: str, date: str, data_path: Optional[str] = None) -> Dict[str, Any]:
        """Load trade history data"""
    ...
    def _validate_performance_requirements(self, trade_data: Dict[str, Any], min_win_rate: float, min_trades: int) -> None:
    ...
    def _has_performance_metrics(self, trade_data: Dict[str, Any]) -> bool:
        ...
        performance_fields = [
            "win_rate", "total_trades", "period_return", "sharpe_ratio",
            "max_drawdown", "profit_factor", "average_win", "average_loss"
        ]
    ...
    def _save_content(self, content: str, analysis_name: str, date: str, output_path: Optional[str] = None) -> Path:
    ...
    def get_performance_summary(self, analysis_name: str, date: str, data_path: Optional[str] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            performance_metrics = {}
            for metric in ["win_rate", "total_trades", "period_return", "sharpe_ratio", "max_drawdown", "profit_factor"]:
    ...
    def validate_trade_data_quality(self, analysis_name: str, date: str, data_path: Optional[str] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            performance_fields = ["win_rate", "total_trades", "period_return", "sharpe_ratio", "max_drawdown"]
            available_performance = [field for field in performance_fields if field in trade_data]
            
            # Check data types
            type_issues = []
            for field, value in trade_data.items():
                if field in ["win_rate", "period_return", "sharpe_ratio", "max_drawdown", "profit_factor"] and not isinstance(value, (int, float)):
            ...
            return {
                ...
                "recommendations": self._generate_quality_recommendations(missing_fields, available_performance, type_issues, logical_issues)
            }
    ...
    def _generate_quality_recommendations(self, missing_fields: List[str], available_performance: List[str], type_issues: List[str], logical_issues: List[str]) -> List[str]:
        ...
        if len(available_performance) < 3:
            recommendations.append("Include more performance metrics (win_rate, total_trades, period_return, sharpe_ratio, max_drawdown)")
