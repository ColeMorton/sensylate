...
class SectorAnalysisScript(BaseScript):
    ...
    def execute(
        ...
        **kwargs
        ...
        try:
            # Validate inputs
            self.validate_inputs(sector_name=sector_name, analysis_type=analysis_type, date=date,
                               data_path=data_path, template_variant=template_variant,
                               output_path=output_path, validate_content=validate_content,
                               min_rotation_signal=min_rotation_signal, benchmark_sectors=benchmark_sectors)
            ...
            self.logger.log_error(
                e,
                {"sector_name": sector_name, "analysis_type": analysis_type, "date": date, "processing_time": processing_time}
            )
    ...
    def _load_sector_data(self, sector_name: str, date: str, data_path: Optional[str] = None) -> Dict[str, Any]:
    ...
    def _determine_analysis_type(self, sector_data: Dict[str, Any]) -> str:
        ...
        rotation_indicators = [
            "rotation_signal", "rotation_score", "flow_data", "economic_cycle",
            "relative_momentum", "sector_rotation"
        ]
        ...
        comparison_indicators = [
            "sector_comparison", "relative_valuation", "performance_ranking",
            "cross_sector", "benchmark_comparison", "peer_sectors"
        ]
        ...
        if rotation_score is not None and isinstance(rotation_score, (int, float)):
            if rotation_score < min_rotation_signal:
                raise ValidationError(
                    ...
                    context={"rotation_score": rotation_score, "threshold": min_rotation_signal}
                )
    
    def _validate_comparison_analysis(self, sector_data: Dict[str, Any], benchmark_sectors: Optional[List[str]]) -> None:
        """Validate comparison analysis requirements"""
        
        # Check for comparison indicators
        comparison_fields = ["sector_comparison", "relative_valuation", "performance_ranking"]
        ...
        if not available_fields:
            raise ValidationError(
                ...
                context={"required_fields": comparison_fields, "available_fields": list(sector_data.keys())}
            )
        ...
        if benchmark_sectors:
            ...
            if sector_name in [s.lower() for s in benchmark_sectors]:
                raise ValidationError(
                    ...
                    context={"sector_name": sector_name, "benchmark_sectors": benchmark_sectors}
                )
    ...
    def _save_content(self, content: str, sector_name: str, analysis_type: str, date: str, output_path: Optional[str] = None) -> Path:
    ...
    def get_sector_analysis_preview(self, sector_name: str, date: str, data_path: Optional[str] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "data_summary": {
                    ...
                    "has_rotation_indicators": any(field in sector_data for field in ["rotation_signal", "rotation_score", "flow_data"]),
                    "has_comparison_indicators": any(field in sector_data for field in ["sector_comparison", "relative_valuation", "performance_ranking"])
                },
