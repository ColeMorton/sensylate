...
class FileOperationManager:
            ...
            with self.acquire_file_lock(file_path):
                ...
                if verify_content:
                    ...
                    if not verification_result.success:
                        raise ProcessingError(
                            f"Content verification failed: {verification_result.error}"
                        )

                # Atomic move: this is the critical operation
                shutil.move(str(temp_path), str(file_path))
                self.logger.debug(
                    f"🔄 Atomically moved temporary file to final location"
                )
                ...
                if file_path.exists():
                    ...
                    self.logger.info(
                        f"📍 Immediate post-move verification: {file_path} ({immediate_size} bytes)"
                    )
                    ...
                    if file_path.exists():
                        ...
                        self.logger.info(
                            f"📍 After 100ms delay verification: {file_path} ({after_delay_size} bytes)"
                        )

                        if immediate_size != after_delay_size:
                            self.logger.error(
                                f"🚨 FILE CORRUPTION DETECTED: Size changed from {immediate_size} to {after_delay_size} bytes in 100ms!"
                            )
                        ...
                        if file_path.exists():
                            ...
                            self.logger.info(
                                f"📍 Final verification: {file_path} ({final_size} bytes)"
                            )

                            if after_delay_size != final_size:
                                self.logger.error(
                                    f"🚨 CONTINUED FILE CORRUPTION: Size changed from {after_delay_size} to {final_size} bytes in additional 200ms!"
                                )

                            self.logger.info(
                                f"✅ Atomic CSV write completed: {file_path} ({final_size} bytes)"
                            )
                            ...
                            raise ProcessingError(
                                "File disappeared during final verification"
                            )
                    else:
                        raise ProcessingError(
                            "File disappeared during delay verification"
                        )
                else:
    ...
    def _verify_csv_content(
        self, file_path: Path, expected_dataframe: pd.DataFrame
    ) -> ProcessingResult:
        """
        ...
        try:
            ...
            if not second_line or len(second_line.split(",")) != len(
    ...
    def safe_file_copy(
        ...
        try:
            ...
            return ProcessingResult(
                success=False, operation="safe_file_copy", error=f"Copy failed: {e}"
            )
