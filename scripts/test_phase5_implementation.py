...
from scripts.test_phase4_implementation import (
    create_test_monthly_data,
...
class Phase5TestSuite:
    ...
    def test_json_schema_system(self) -> Dict[str, Any]:
        ...
        try:
            ...
            missing_schemas = [
                name for name in required_schemas if name not in all_schemas
            ]
            assert len(missing_schemas) == 0, f"Missing schemas: {missing_schemas}"

            # Test schema export
            exported_files = self.schema_generator.export_schemas(
                "data/outputs/test_schemas"
            )
            assert len(exported_files) == len(
                all_schemas
            ), "Schema export count mismatch"
            ...
            for chart_type, example_config in examples.items():
                is_valid, errors = self.schema_generator.validate_chart_config(
                    example_config, chart_type
                )
                if not is_valid:
                    print(
                        f"Warning: Example config validation failed for {chart_type}: {errors}"
                    )
            ...
            print(
                f"  ✅ JSON Schema System: {len(all_schemas)} schemas, {len(exported_files)} files exported"
            )

        except Exception as e:
            results["json_schema_system"] = {"status": f"❌ FAIL: {str(e)}"}
    ...
    def test_frontend_config_export(self) -> Dict[str, Any]:
        ...
        try:
            ...
            donut_config = self.config_exporter.export_chart_config(
                chart_type="enhanced_donut_chart", data=quality_data, theme_mode="light"
            )
            ...
            waterfall_config = self.config_exporter.export_chart_config(
                chart_type="waterfall_chart", data=trade_data[:20], theme_mode="light"
            )
            chart_configs["waterfall"] = waterfall_config

            # Test scatter chart export
            scatter_config = self.config_exporter.export_chart_config(
                chart_type="enhanced_scatter", data=trade_data, theme_mode="light"
            )
            ...
            for name, filepath in exported_files.items():
                ...
                with open(filepath, "r") as f:
                    ...
                    assert (
                        "chart_type" in config_content
                    ), f"Invalid config file: {name}"
            ...
            print(
                f"  ✅ Frontend Config Export: {len(chart_configs)} configs, {len(exported_files)} files"
            )

        except Exception as e:
            results["frontend_config_export"] = {"status": f"❌ FAIL: {str(e)}"}
    ...
    def test_production_optimization(self) -> Dict[str, Any]:
        ...
        try:
            ...
            assert (
                self.production_optimizer is not None
            ), "Production optimizer not initialized"

            # Test template caching
            self.production_optimizer.template_cache.set_template(
                "test", "light", "template_obj"
            )
            cached_template = self.production_optimizer.template_cache.get_template(
                "test", "light"
            )
            ...
            batch_results = self.production_optimizer.batch_optimize_charts(
                chart_requests
            )
            ...
            assert (
                "total_charts_generated" in performance_report["summary"]
            ...
            results["production_optimization"] = {"status": f"❌ FAIL: {str(e)}"}
    ...
    def test_feature_flag_system(self) -> Dict[str, Any]:
        ...
        try:
            ...
            assert (
                65 <= rollout_percentage <= 85
            ), f"Rollout percentage off: {rollout_percentage}%"
            ...
            assert self.feature_flag_manager.is_enabled(
                "conditional_test", prod_context
            ), "Conditional flag failed"
            assert not self.feature_flag_manager.is_enabled(
                "conditional_test", dev_context
            ), "Conditional flag leaked"
            ...
            print(
                f"  ✅ Feature Flag System: {len(enabled_features)} enabled, rollout {rollout_percentage:.1f}%"
            )

        except Exception as e:
            results["feature_flag_system"] = {"status": f"❌ FAIL: {str(e)}"}
        ...
        try:
            ...
            try:
                ...
                chart_tests["waterfall_chart"] = {
                    ...
                    "trace_count": len(waterfall_result.data)
                    if waterfall_result
                    else 0,
                }
            except Exception as e:
            ...
            for chart_type in ["monthly_bars", "donut_charts", "waterfall", "scatter"]:
                ...
                integration_tests[chart_type] = {
                    ...
                    "flag_exists": self.feature_flag_manager.get_flag(flag_name)
                    is not None,
                }

            # Calculate success rates
            chart_success_rate = sum(
                1
                for test in chart_tests.values()
                if test.get("generation_successful", False)
            ) / len(chart_tests)

            theme_success_rate = sum(
                1 for test in theme_tests.values() if "error" not in test
            ) / len(theme_tests)

            export_success_rate = sum(
                1 for test in export_tests.values() if "error" not in test
            ) / len(export_tests)

            integration_success_rate = sum(
                1
                for test in integration_tests.values()
                if test.get("flag_enabled", False)
            ...
            print(
                f"  ✅ Comprehensive Regression: {overall_success_rate * 100:.1f}% success rate"
            )

        except Exception as e:
            results["comprehensive_regression"] = {"status": f"❌ FAIL: {str(e)}"}
    ...
    def run_comprehensive_tests(self) -> Dict[str, Any]:
        ...
        print(
            f"🎯 Status: {'✅ PHASE 5 COMPLETE' if pass_rate >= 90 else '❌ NEEDS ATTENTION'}"
        )
