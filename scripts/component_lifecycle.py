...
class ComponentLifecycle(ABC):
    ...
    def init(self) -> None:
        ...
        if self.state != ComponentState.UNINITIALIZED:
            raise RuntimeError(
                f"Component {self.component_name} already initialized (state: {self.state})"
            )
        ...
        try:
            ...
            self.logger.error(
                f"Component {self.component_name} initialization failed: {e}"
            )
            raise
    ...
    def configure(self, force_reconfigure: bool = False) -> None:
        ...
        if self.state == ComponentState.UNINITIALIZED:
            raise RuntimeError(
                f"Component {self.component_name} must be initialized before configuration"
            )

        if self.state == ComponentState.FAILED:
            raise RuntimeError(
                f"Component {self.component_name} is in failed state and cannot be configured"
            )
        ...
        if self.is_configured() and self._cache_valid and not force_reconfigure:
            self.logger.debug(
                f"Component {self.component_name} already configured, using cached configuration"
            )
            return
        ...
        try:
            ...
            self.logger.debug(
                f"Component {self.component_name} configured successfully"
            )

        except Exception as e:
            self.state = ComponentState.FAILED
            self.logger.error(
                f"Component {self.component_name} configuration failed: {e}"
            )
            raise
    ...
    def start(self) -> None:
        ...
        if not self.is_configured():
            raise RuntimeError(
                f"Component {self.component_name} must be configured before starting"
            )

        if self.state == ComponentState.FAILED:
            raise RuntimeError(
                f"Component {self.component_name} is in failed state and cannot be started"
            )
    ...
    def stop(self) -> None:
        ...
        if self.state not in [ComponentState.STARTED, ComponentState.CONFIGURED]:
            self.logger.warning(
                f"Component {self.component_name} not running (state: {self.state})"
            )
            return
    ...
    def get_lifecycle_info(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "configuration_time": self.configuration_time.isoformat()
...
class ComponentLifecycleManager:
    ...
    def register_component(
        self, component: ComponentLifecycle, dependencies: list[str] = None
    ) -> None:
        """
        ...
        self.logger.debug(
            f"Registered component {component.component_name} with dependencies: {dependencies}"
        )
