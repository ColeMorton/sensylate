...
Usage:
    python scripts/generate_report_with_dashboard.py \\
        --config configs/report_generation.yaml \\
...
class IntegratedReportGenerator:
    ...
    def generate_integrated_report(self, input_file: Path, output_dir: Optional[Path] = None) -> Dict[str, List[Path]]:
        """
    ...
    def _find_historical_performance_files(self) -> List[Path]:
        ...
        patterns = [
            "HISTORICAL_PERFORMANCE_REPORT_*.md",
            "historical_performance_*.md"
        ]
...
def main(report_config: Dict[str, Any], dashboard_config: Dict[str, Any], 
         input_file: Path, output_dir: Optional[Path] = None) -> Dict[str, List[Path]]:
    """
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--input",
        required=True,
        help="Input data file for report generation"
    )
    parser.add_argument(
        "--output-dir",
        help="Output directory override (default from configs)"
    )
    parser.add_argument(
        "--env",
