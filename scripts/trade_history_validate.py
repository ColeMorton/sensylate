...
class TradeHistoryValidator:
    ...
    def __init__(self, portfolio: str, validation_depth: str = "standard", confidence_threshold: float = 0.7):
    ...
    def load_phase_outputs(self) -> Dict[str, Any]:
        ...
        try:
            # Load discovery data
            discovery_path = base_path / "discovery" / f"{self.portfolio}_{date_suffix}.json"
            with open(discovery_path, 'r') as f:
                self.discovery_data = json.load(f)
                
            # Load analysis data  
            analysis_path = base_path / "analysis" / f"{self.portfolio}_{date_suffix}.json"
            with open(analysis_path, 'r') as f:
                self.analysis_data = json.load(f)
                
            # Load synthesis data (from internal report)
            synthesis_path = base_path / "internal" / f"{self.portfolio}_{date_suffix}.md"
    ...
    def validate_statistical_calculations(self) -> Dict[str, Any]:
        ...
        try:
            risk_metrics = self.analysis_data.get("performance_measurement", {}).get("statistical_analysis", {}).get("risk_adjusted_metrics", {})
            reported_sharpe = risk_metrics.get("sharpe_ratio", 0)
            
            # Cross-validate against return and volatility
            return_stats = self.analysis_data.get("performance_measurement", {}).get("statistical_analysis", {}).get("return_distribution", {})
            ...
            if std_dev > 0:
                ...
                tolerance_met = variance <= 2.0  # Allow some variance in different calculation methods
        ...
        try:
            total_trades = self.discovery_data.get("portfolio_summary", {}).get("total_trades", 0)
        ...
        overall_confidence = sum(individual_confidences) / len(individual_confidences) if individual_confidences else 0.0
    ...
    def validate_report_integrity(self) -> Dict[str, Any]:
        ...
        try:
            if self.synthesis_data and self.synthesis_data.get("internal_report_content"):
        ...
        try:
            if self.discovery_data and self.analysis_data:
                ...
                discovery_total_trades = self.discovery_data.get("portfolio_summary", {}).get("total_trades", 0)
                
                analysis_trades = self.analysis_data.get("statistical_validation", {}).get("sample_size_assessment", {}).get("total_trades", 0)
                ...
                integrity_results["content_accuracy"] = {
                    ...
                    "trade_count_variance": abs(discovery_total_trades - analysis_trades),
        ...
        overall_integrity_confidence = sum(integrity_confidences) / len(integrity_confidences) if integrity_confidences else 0.0
    ...
    def validate_business_logic(self) -> Dict[str, Any]:
        ...
        try:
            ...
            strategy_data = signal_data.get("entry_signal_analysis", {}).get("win_rate_by_strategy", {})
            
            coherence_checks = {"win_rate_bounds": True, "confidence_scores_valid": True}
            ...
            business_logic_results["signal_effectiveness_coherence"] = {
                ...
                "validation_confidence": 0.90 if all(coherence_checks.values()) else 0.70
            }
        ...
        try:
            ...
            feasibility_scores = []
            for category in ["entry_signal_enhancements", "exit_signal_refinements", "strategy_parameter_optimization"]:
                opportunities = optimizations.get(category, [])
                
                for opp in opportunities:
                    confidence = opp.get("confidence", 0) if isinstance(opp, dict) else 0
                    if 0 <= confidence <= 1:
            ...
            business_logic_results["optimization_feasibility"] = {
                ...
                "feasibility_assessment": "High" if avg_feasibility > 0.75 else "Moderate" if avg_feasibility > 0.5 else "Low",
        ...
        try:
            ...
            correlation_bounds_valid = (-1 <= avg_correlation <= 1) and (-1 <= max_correlation <= 1)
            ...
            business_logic_results["risk_assessment_coherence"] = {
                ...
                "validation_confidence": 0.88 if all([correlation_bounds_valid, diversification_valid, logical_consistency]) else 0.65
            }
        ...
        overall_business_confidence = sum(business_confidences) / len(business_confidences) if business_confidences else 0.0
        business_logic_results["overall_business_logic_confidence"] = overall_business_confidence
        
        return business_logic_results
    
    def calculate_comprehensive_confidence(self, statistical_results: Dict, integrity_results: Dict, business_logic_results: Dict) -> Dict[str, Any]:
        ...
        discovery_confidence = self.discovery_data.get("discovery_metadata", {}).get("confidence_score", 0.0) if self.discovery_data else 0.0
        analysis_confidence = self.analysis_data.get("analysis_quality_assessment", {}).get("overall_confidence", 0.0) if self.analysis_data else 0.0
        
        statistical_confidence = statistical_results.get("overall_statistical_confidence", 0.0)
        integrity_confidence = integrity_results.get("overall_integrity_confidence", 0.0)
        business_logic_confidence = business_logic_results.get("overall_business_logic_confidence", 0.0)
        
        # Weighted aggregation (Discovery: 25%, Analysis: 40%, Synthesis: 35%)
        synthesis_confidence = (integrity_confidence + business_logic_confidence) / 2  # Combine integrity and business logic for synthesis
    ...
    def generate_validation_report(self, statistical_results: Dict, integrity_results: Dict, 
                                 business_logic_results: Dict, confidence_scoring: Dict) -> Dict[str, Any]:
        ...
        quality_gates = {
            "statistical_validation": statistical_results.get("overall_statistical_confidence", 0.0) >= 0.80,
            "report_integrity": integrity_results.get("overall_integrity_confidence", 0.0) >= 0.85,
            "business_logic": business_logic_results.get("overall_business_logic_confidence", 0.0) >= 0.80,
        ...
        if not quality_gates["statistical_validation"]:
            validation_summary["critical_issues"].append("Statistical validation below threshold - review calculation accuracy")
            validation_summary["recommendations"].append("Enhance statistical validation methodology and cross-validation procedures")
            
        if not quality_gates["report_integrity"]:
            validation_summary["critical_issues"].append("Report integrity issues detected - review content consistency")
            validation_summary["recommendations"].append("Improve report generation and content validation processes")
            
        if not quality_gates["business_logic"]:
            validation_summary["critical_issues"].append("Business logic coherence issues - review optimization feasibility")
            validation_summary["recommendations"].append("Strengthen business logic validation and coherence checking")
    ...
    def execute_validation(self) -> Dict[str, Any]:
        ...
        if not all([load_results.get("discovery_loaded"), load_results.get("analysis_loaded")]):
            return {"error": "Failed to load required phase outputs", "details": load_results}
            
        print(f"✅ Discovery: {'Loaded' if load_results.get('discovery_loaded') else 'Failed'}")
        print(f"✅ Analysis: {'Loaded' if load_results.get('analysis_loaded') else 'Failed'}")
        print(f"✅ Synthesis: {'Loaded' if load_results.get('synthesis_loaded') else 'Failed'}")
        ...
        business_confidence = business_logic_results.get("overall_business_logic_confidence", 0.0)
        ...
        validation_report = self.generate_validation_report(
            statistical_results, integrity_results, business_logic_results, confidence_scoring
        )
    ...
    def save_validation_report(self, validation_report: Dict[str, Any]) -> str:
        ...
        output_dir = Path("/Users/colemorton/Projects/sensylate/data/outputs/trade_history/validation")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Trade History Validation - DASV Phase 4")
    parser.add_argument("portfolio", help="Portfolio name to validate")
    parser.add_argument("--validation_depth", choices=["basic", "standard", "comprehensive", "institutional"], 
    ...
    try:
        ...
        quality_cert = summary.get("quality_certification", "unknown").replace('_', ' ').title()
