...
from result_types import ProcessingResult, ErrorResult
...
class ScriptRegistry:
    ...
    def register_script(self, script_class: Type[BaseScript], name: Optional[str] = None) -> None:
        ...
        try:
            ...
            self.logger.log_operation(
                ...
                {"script_class": script_class.__name__, "metadata": instance.metadata.to_dict()}
            )
    ...
    def execute_script(self, script_name: str, **kwargs) -> ProcessingResult:
        ...
        if not script:
            return ErrorResult.from_exception(
                ...
                "script_execution",
                {"script_name": script_name, "available_scripts": self.list_available_scripts()}
            )
    ...
    def export_registry_info(self, output_file: Path) -> None:
        ...
        registry_info = {
            ...
            "scripts": {
                ...
                for name, metadata in self._scripts.items()
            },
            "content_type_support": self._get_content_type_support(),
            "export_timestamp": TwitterSystemLogger().logger.handlers[0].formatter.formatTime(
                None, None
            ) if TwitterSystemLogger().logger.handlers else "unknown"
        }
        ...
        for search_path in search_paths:
            ...
            for python_file in search_path.rglob("*.py"):
                ...
                try:
                    ...
                    self.logger.log_error(
                        e,
                        {"file": str(python_file), "search_path": str(search_path)}
                    )
    ...
    def get_registry_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            ) / len(self._scripts) if self._scripts else 0
        }
