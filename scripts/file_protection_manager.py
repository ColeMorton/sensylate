...
class FileProtectionManager:
    ...
    def protected_write_csv(self, df: pd.DataFrame, file_path: Path, timeout: int = 30) -> bool:
        """
        ...
        self.logger.info(f"🛡️ Starting protected write for {file_path} with {len(df)} rows")
            ...
            with open(lock_path, 'w') as lock_file:
                try:
                    ...
                    while time.time() - start_time < timeout:
                        ...
                        self.logger.error(f"Failed to acquire file lock for {file_path} within {timeout}s")
                        return False
                    ...
                    if not self._verify_csv_integrity(temp_path, df):
                        ...
                        self.logger.error(f"Temporary file integrity check failed: {temp_path}")
                        return False
                    ...
                    if not file_path.exists():
                        self.logger.error(f"File does not exist after atomic move: {file_path}")
                        return False
                    
                    # Verify final file integrity
                    if not self._verify_csv_integrity(file_path, df):
                        self.logger.error(f"Final file integrity check failed: {file_path}")
                        return False
                    ...
                    self.logger.info(f"✅ Protected CSV write completed: {file_path} ({file_size} bytes)")
                    return True
    ...
    def _verify_csv_integrity(self, file_path: Path, expected_df: pd.DataFrame) -> bool:
        ...
        try:
            ...
            if read_df.shape != expected_df.shape:
                self.logger.error(f"DataFrame shape mismatch: expected {expected_df.shape}, got {read_df.shape}")
                return False
            
            # Check columns match
            if list(read_df.columns) != list(expected_df.columns):
                self.logger.error(f"Column mismatch: expected {list(expected_df.columns)}, got {list(read_df.columns)}")
                return False
    ...
    def auto_recover_from_backup(self, file_path: Path) -> bool:
        ...
        try:
            ...
            if backup_size < 100:
                self.logger.error(f"🔧 Backup file too small ({backup_size} bytes): {backup_path}")
                return False
            
            # Log corruption pattern analysis
            self._log_corruption_analysis(file_path, backup_path)
            
            self.logger.info(f"🔄 Auto-recovering from backup: {backup_path} ({backup_size} bytes)")
            ...
            if file_path.exists() and file_path.stat().st_size > 100:
                ...
                self.logger.info(f"✅ Auto-recovery successful: {file_path} ({restored_size} bytes)")
                
                # Log recovery success pattern
                self.logger.info(f"📊 Corruption pattern: External process truncated {restored_size} bytes → 1 byte, recovered successfully")
                return True
            else:
    ...
    def _log_corruption_analysis(self, file_path: Path, backup_path: Path) -> None:
        ...
        try:
            ...
            corruption_time = backup_stat.st_mtime - file_stat.st_mtime if file_stat.st_mtime > 0 else "unknown"
            
            self.logger.warning(f"🔍 Corruption Analysis: {file_path.name}")
            self.logger.warning(f"   └ Backup size: {backup_stat.st_size:,} bytes | Corrupted size: {file_stat.st_size} bytes")
            self.logger.warning(f"   └ Corruption pattern: Complete data loss (likely external process interference)")
    ...
    def _detect_suspicious_processes(self, file_path: Path) -> None:
        ...
        try:
            ...
            if active_suspicious:
                self.logger.warning(f"   └ Potentially interfering processes detected: {', '.join(active_suspicious)}")
                self.logger.warning(f"   └ Recommendation: Consider excluding {file_path.parent} from Spotlight indexing")
            else:
                self.logger.warning(f"   └ No obvious interfering processes detected (corruption timing suggests external interference)")
    ...
    def _apply_protection_attributes(self, file_path: Path) -> None:
        ...
        try:
            ...
            protected_mode = current_mode & ~stat.S_IWUSR & ~stat.S_IWGRP & ~stat.S_IWOTH
    ...
    def check_file_integrity(self, file_path: Path) -> bool:
        ...
        try:
            ...
            if current_size < 100:
                self.logger.error(f"🚨 External corruption detected: {file_path} ({current_size} bytes)")
                return self.auto_recover_from_backup(file_path)
