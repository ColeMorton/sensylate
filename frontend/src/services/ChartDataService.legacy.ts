...
class LegacyChartDataService {
  ...
  async fetchPortfolioData(signal?: AbortSignal): Promise<PortfolioDataRow[]> {
    return unifiedChartDataService.fetchChartData('portfolio-value-comparison', signal);
  }

  async fetchLiveSignalsData(signal?: AbortSignal): Promise<LiveSignalsDataRow[]> {
    return unifiedChartDataService.fetchChartData('live-signals-equity-curve', signal);
  }

  async fetchTradeHistoryData(signal?: AbortSignal): Promise<TradeHistoryDataRow[]> {
    return unifiedChartDataService.fetchChartData('trade-pnl-waterfall', signal);
  }

  async fetchClosedPositionsPnLData(signal?: AbortSignal): Promise<ClosedPositionPnLDataRow[]> {
    return unifiedChartDataService.fetchChartData('closed-positions-pnl-timeseries', signal);
  }

  async fetchOpenPositionsPnLData(signal?: AbortSignal): Promise<OpenPositionPnLDataRow[]> {
    return unifiedChartDataService.fetchChartData('open-positions-pnl-timeseries', signal);
  }
  ...
  async fetchLiveSignalsBenchmarkData(signal?: AbortSignal): Promise<LiveSignalsBenchmarkDataRow[]> {
    return unifiedChartDataService.fetchChartData('live-signals-benchmark-comparison', signal);
  }
