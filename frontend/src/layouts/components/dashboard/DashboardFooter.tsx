...
const DashboardFooter: React.FC<DashboardFooterProps> = ({
  ...
  const textClasses =
    "brand-text text-text-dark dark:text-darkmode-text-dark m-0 mb-8 text-4xl font-semibold";
  const containerClasses = config.className
    ? `${baseClasses} ${config.className}`
    : baseClasses;

  return (
    <div className={containerClasses}>
      <h1 className={textClasses}>{config.text}</h1>
    </div>
  );
};
