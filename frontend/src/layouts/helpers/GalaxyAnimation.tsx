...
interface GalaxyAnimationRef {
  setOpacity: (opacity: number) => void;
  getOpacity: () => number;
}

const GalaxyAnimation = React.memo(
  forwardRef<GalaxyAnimationRef, GalaxyAnimationProps>(
    ({ className = "", onReady }, ref) => {
      ...
      useEffect(() => {
        ...
        try {
          ...
          canvasRef.current = refs.renderer.domElement;
          canvasRef.current.style.opacity = opacityRef.current.toString();
        ...
        // Notify parent component that galaxy is ready
        onReady?.();
      ...
      useImperativeHandle(
        ...
        () => ({
          setOpacity: (opacity: number) => {
            ...
            // Control canvas opacity directly
            if (canvasRef.current) {
              canvasRef.current.style.opacity = opacity.toString();
            } else if (containerRef.current) {
              // Fallback to container if canvas not ready
...
export type { GalaxyAnimationRef };
