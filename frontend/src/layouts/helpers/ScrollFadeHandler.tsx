...
const ScrollFadeHandler: React.FC<ScrollFadeHandlerProps> = ({
  ...
  useEffect(() => {
    ...
    if (!galaxyElement) {
      throw new Error(`ScrollFadeHandler: Element with ID "${galaxyId}" not found`);
    }
    if (!textElement) {
      throw new Error(`ScrollFadeHandler: Element with ID "${textId}" not found`);
    }

    const handleWheel = (event: WheelEvent) => {
      // Calculate text opacity (fades twice as fast)
      const textOpacity = Math.max(0, 1 - (scrollProgressRef.current * 2 / fadeDistance));
      ...
      animationFrameRef.current = requestAnimationFrame(() => {
        ...
        scrollProgressRef.current = Math.max(0, Math.min(maxProgress, scrollProgressRef.current));
        
        // Calculate text opacity (fades twice as fast, reaches 0 at 50% progress)
        const newTextOpacity = Math.max(0, 1 - (scrollProgressRef.current * 2 / fadeDistance));
        
        // Calculate galaxy opacity (fades normally, reaches 0.5 at 100% progress)
        const galaxyOpacity = Math.max(0.5, 1 - (scrollProgressRef.current / (fadeDistance / 2)));
