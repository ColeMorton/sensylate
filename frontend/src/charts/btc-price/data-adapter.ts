...
export interface BTCPriceDataAdapter extends ChartDataAdapter<StockDataRow> {
  fetchData(signal?: AbortSignal, params?: any): Promise<StockDataRow[]>;
}
...
export class BTCPriceDataAdapterImpl implements BTCPriceDataAdapter {
  ...
  async fetchData(signal?: AbortSignal, params?: any): Promise<StockDataRow[]> {
    ...
    try {
      ...
      throw new Error(`BTC data fetch failed: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }
