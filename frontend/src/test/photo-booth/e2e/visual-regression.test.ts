...
describe('Visual Regression Tests', () => {
  ...
  describe('Screenshot Baseline Management', () => {
    ...
    it('creates baseline screenshots for all aspect ratio and theme combinations', async () => {
      ...
      for (const combo of combinations) {
        await page.goto(`${baseURL}/photo-booth?dashboard=portfolio_history_portrait&aspect_ratio=${combo.aspectRatio}&mode=${combo.mode}`, {
    ...
    it('validates portfolio history portrait header consistency across themes', async () => {
      ...
      for (const mode of modes) {
        await page.goto(`${baseURL}/photo-booth?dashboard=portfolio_history_portrait&aspect_ratio=3:4&mode=${mode}`, {
          waitUntil: 'networkidle0',
          timeout: 15000
        });
        ...
        const headerText = await page.$eval('.dashboard-header h1', el => el.textContent);
    ...
    it('validates footer consistency across aspect ratios', async () => {
      ...
      for (const ratio of aspectRatios) {
        await page.goto(`${baseURL}/photo-booth?dashboard=portfolio_history_portrait&aspect_ratio=${ratio}&mode=light`, {
          waitUntil: 'networkidle0',
          timeout: 15000
        });
        ...
        const footerText = await page.$eval('.dashboard-footer h1', el => el.textContent);
  ...
  describe('Chart Visual Consistency', () => {
    it('validates chart rendering consistency across themes', async () => {
      ...
      for (const mode of modes) {
        await page.goto(`${baseURL}/photo-booth?dashboard=portfolio_history_portrait&aspect_ratio=16:9&mode=${mode}`, {
          waitUntil: 'networkidle0',
          timeout: 15000
        });
    ...
    it('validates chart layout in portrait orientation', async () => {
      ...
      await page.goto(`${baseURL}/photo-booth?dashboard=portfolio_history_portrait&aspect_ratio=3:4&mode=light`, {
        waitUntil: 'networkidle0',
        timeout: 15000
      });
      ...
      const chartsContainer = await page.$('.flex.flex-col.h-full.min-h-0.flex-1');
      ...
      expect(chart2Box!.y).toBeGreaterThan(chart1Box!.y + chart1Box!.height - 50); // Small overlap tolerance
    });
  });
  ...
  describe('Export Mode Visual Validation', () => {
    it('validates clean export screenshots without UI controls', async () => {
      ...
      for (const config of configurations) {
        await page.goto(`${baseURL}/photo-booth?dashboard=portfolio_history_portrait&aspect_ratio=${config.aspectRatio}&mode=${config.mode}`, {
          waitUntil: 'networkidle0',
          timeout: 15000
        });
        ...
        await page.evaluate(() => {
          ...
          const devElements = document.querySelectorAll('[data-astro-dev-toolbar], astro-dev-toolbar, #dev-toolbar-root');
        ...
        const visibleControls = await page.$$eval('.photo-booth-controls', controls => 
  ...
  describe('Cross-Browser Visual Consistency', () => {
    it('validates rendering consistency across different browser settings', async () => {
      ...
      for (const scaleFactor of scaleFactors) {
        ...
        await page.goto(`${baseURL}/photo-booth?dashboard=portfolio_history_portrait&aspect_ratio=16:9&mode=light`, {
          waitUntil: 'networkidle0',
          timeout: 15000
        });
