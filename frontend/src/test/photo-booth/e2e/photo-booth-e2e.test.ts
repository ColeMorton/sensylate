...
describe('Photo Booth E2E Tests', () => {
  ...
  describe('Page Loading and Navigation', () => {
    it('loads photo booth page successfully', async () => {
      ...
      const footer = await page.$eval('.dashboard-footer h1', el => el.textContent);
    ...
    it('loads specific dashboard via URL parameters', async () => {
      ...
      await page.goto(`${baseURL}/photo-booth?dashboard=portfolio_history_portrait&mode=light&aspect_ratio=3:4`, {
        waitUntil: 'networkidle0',
        timeout: 15000
      });
      ...
      const selectedDashboard = await page.$eval('#dashboard-select', el => (el as HTMLSelectElement).value);
    ...
    it('handles malformed URL parameters gracefully', async () => {
      ...
      await page.goto(`${baseURL}/photo-booth?dashboard=invalid&mode=invalid&aspect_ratio=invalid`, {
        waitUntil: 'networkidle0',
        timeout: 15000
      });
  ...
  describe('Dashboard Interaction', () => {
    ...
    it('switches between light and dark modes', async () => {
      ...
      await page.waitForSelector('.photo-booth-dashboard.dark', { timeout: 5000 });
      
      await e2eHelper.takeScreenshot(page, 'dashboard-dark-mode');

      // Verify dark mode is applied
      const isDark = await page.$eval('.photo-booth-dashboard', el => el.classList.contains('dark'));
      ...
      // Wait for theme change
      await page.waitForSelector('.photo-booth-dashboard:not(.dark)', { timeout: 5000 });

      // Verify light mode is applied
      const isLight = await page.$eval('.photo-booth-dashboard', el => !el.classList.contains('dark'));
    ...
    it('changes aspect ratios and updates display', async () => {
      ...
      for (const ratio of aspectRatios) {
        ...
        await e2eHelper.takeScreenshot(page, `dashboard-${ratio.replace(':', 'x')}-aspect-ratio`);
    ...
    it('updates export parameters dynamically', async () => {
      ...
      const outputInfo = await page.$eval('.ml-auto.text-xs', el => el.textContent);
  ...
  describe('Dashboard Content Verification', () => {
    ...
    it('renders portfolio history portrait with correct structure', async () => {
      ...
      await e2eHelper.takeScreenshot(page, 'portfolio-history-portrait-structure');
    });
  ...
  describe('Responsive Behavior', () => {
    it('adapts to different viewport sizes', async () => {
      ...
      for (const viewport of viewports) {
        ...
        await page.goto(`${context.baseURL}/photo-booth?dashboard=portfolio_history_portrait&aspect_ratio=3:4`, {
          waitUntil: 'networkidle0',
          timeout: 15000
        });

        await page.waitForSelector('.photo-booth-ready', { timeout: 20000 });
        await e2eHelper.takeScreenshot(page, `responsive-${viewport.name}-${viewport.width}x${viewport.height}`);
    ...
    it('handles mobile viewport gracefully', async () => {
      ...
      await page.goto(`${context.baseURL}/photo-booth?dashboard=portfolio_history_portrait`, {
        waitUntil: 'networkidle0',
        timeout: 15000
      });
