...
describe("Aspect Ratio Validation E2E Tests", () => {
  ...
  describe("Aspect Ratio Dimension Validation", () => {
    ...
    aspectRatios.forEach(({ id, name, width, height }) => {
      it(`validates ${id} aspect ratio dimensions and layout`, async () => {
        ...
        await page.goto(
          `${baseURL}/photo-booth?dashboard=portfolio_history_portrait&aspect_ratio=${id}`,
          {
        ...
        await e2eHelper.takeScreenshot(
          page,
          `aspect-ratio-${id.replace(":", "x")}-${name.toLowerCase()}`,
        );
    ...
    it("compares all aspect ratios visually for layout consistency", async () => {
      ...
      for (const ratio of aspectRatios) {
        await page.goto(
          `${baseURL}/photo-booth?dashboard=portfolio_history_portrait&aspect_ratio=${ratio.id}&mode=light`,
          {
            waitUntil: "networkidle0",
            timeout: 15000,
          },
    ...
    aspectRatios.forEach((aspectRatio) => {
      modes.forEach((mode) => {
        it(`validates ${aspectRatio} aspect ratio in ${mode} mode`, async () => {
          ...
          await page.goto(
            `${baseURL}/photo-booth?dashboard=portfolio_history_portrait&aspect_ratio=${aspectRatio}&mode=${mode}`,
            {
              waitUntil: "networkidle0",
              timeout: 15000,
            },
          ...
          const dashboard = await page.$(".photo-booth-dashboard");
          const hasDarkClass = await dashboard?.evaluate((el) =>
          ...
          const buttonText = await activeButton?.evaluate((el) =>
            el.textContent?.trim(),
          );
          ...
          await e2eHelper.takeScreenshot(
            page,
            `${aspectRatio.replace(":", "x")}-${mode}-combination`,
          );
    ...
    it("smoothly transitions between aspect ratios", async () => {
      ...
      for (let i = 0; i < ratios.length; i++) {
        ...
        if (i > 0) {
          await e2eHelper.takeScreenshot(
            page,
            `before-switch-to-${ratio.replace(":", "x")}`,
          );
        }
        ...
        const loadingIndicator = await page
          .waitForSelector("text=Loading...", { timeout: 2000 })
          .catch(() => null);
        if (loadingIndicator) {
          await e2eHelper.takeScreenshot(
            page,
            `loading-state-${ratio.replace(":", "x")}`,
          );
        }
        ...
        await e2eHelper.takeScreenshot(
          page,
          `after-switch-to-${ratio.replace(":", "x")}`,
        );
  ...
  describe("Export Mode Dimension Validation", () => {
    it("validates export mode CSS behavior for different aspect ratios", async () => {
      ...
      for (const ratio of aspectRatios) {
        await page.goto(
          `${baseURL}/photo-booth?dashboard=portfolio_history_portrait&aspect_ratio=${ratio}`,
          {
            waitUntil: "networkidle0",
            timeout: 15000,
          },
  ...
  describe("Chart Adaptation to Aspect Ratios", () => {
    it("validates charts adapt properly to portrait vs landscape ratios", async () => {
      ...
      for (const ratio of ratios) {
        await page.goto(
          `${baseURL}/photo-booth?dashboard=portfolio_history_portrait&aspect_ratio=${ratio.id}`,
          {
            waitUntil: "networkidle0",
            timeout: 15000,
          },
        ...
        await e2eHelper.takeScreenshot(
          page,
          `charts-${ratio.orientation}-${ratio.id.replace(":", "x")}`,
        );
        ...
        for (const chart of charts) {
          ...
          const dashboardBox = await page.$eval(
          ...
          expect(chartBox!.x + chartBox!.width).toBeLessThanOrEqual(
            dashboardBox.x + dashboardBox.width + 10,
          );
        }
      }
    });
    ...
    it("validates chart responsiveness during rapid aspect ratio changes", async () => {
      ...
      for (let i = 0; i < ratios.length; i++) {
        ...
        await e2eHelper.takeScreenshot(
          page,
          `rapid-change-${i}-${ratio.replace(":", "x")}`,
        );
      }
  ...
  describe("Aspect Ratio Edge Cases", () => {
    it("handles invalid aspect ratio parameters gracefully", async () => {
      ...
      for (const invalidRatio of invalidRatios) {
        await page.goto(
          `${baseURL}/photo-booth?dashboard=portfolio_history_portrait&aspect_ratio=${invalidRatio}`,
          {
            waitUntil: "networkidle0",
            timeout: 15000,
          },
        ...
        await e2eHelper.takeScreenshot(
          page,
          `invalid-ratio-fallback-${invalidRatio.replace(/[^a-zA-Z0-9]/g, "_")}`,
        );
      }
    });
    ...
    it("validates consistent behavior across browser refreshes", async () => {
      ...
      await page.goto(
        `${baseURL}/photo-booth?dashboard=portfolio_history_portrait&aspect_ratio=${testRatio}`,
        {
          waitUntil: "networkidle0",
          timeout: 15000,
        },
      ...
      // Get initial state
      const initialRatio = await page.$eval(
        "#aspect-ratio-select",
        (el) => (el as HTMLSelectElement).value,
      );
      const initialHeader = await page.$eval(
        ".dashboard-header h1",
        (el) => el.textContent,
      );
      ...
      const afterRefreshRatio = await page.$eval(
        "#aspect-ratio-select",
        (el) => (el as HTMLSelectElement).value,
      );
      const afterRefreshHeader = await page.$eval(
        ".dashboard-header h1",
        (el) => el.textContent,
      );
