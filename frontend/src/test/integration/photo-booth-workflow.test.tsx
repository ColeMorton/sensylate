...
describe('Photo Booth Workflow Integration', () => {
  ...
  beforeEach(() => {
    ...
    // Mock successful dashboard loading
  ...
  describe('Complete Dashboard Loading Flow', () => {
    it('loads dashboard, shows loading state, then renders content', async () => {
      ...
      // Charts are rendered
      expect(screen.getByTestId('mock-chart-trade-pnl-waterfall')).toBeInTheDocument();
      expect(screen.getByTestId('mock-chart-closed-positions-pnl-timeseries')).toBeInTheDocument();
    ...
    it('handles dashboard loading failure and retry', async () => {
      ...
      mockDashboardLoader.getAllDashboards.mockRejectedValueOnce(new Error('Network error'));
  ...
  describe('Parameter Synchronization Workflow', () => {
    ...
    it('maintains parameter consistency across state changes', async () => {
      ...
      await waitFor(() => {
        ...
        expect(screen.getByDisplayValue('Portfolio History Portrait')).toBeInTheDocument();
  ...
  describe('Aspect Ratio Workflow', () => {
    it('changes aspect ratio and updates dimensions', async () => {
      ...
      await waitFor(() => {
        expect(mockSetProperty).toHaveBeenCalledWith('--photo-booth-width', '1440px');
        expect(mockSetProperty).toHaveBeenCalledWith('--photo-booth-height', '1080px');
      });
      ...
      await waitFor(() => {
        expect(mockSetProperty).toHaveBeenCalledWith('--photo-booth-width', '1920px');
        expect(mockSetProperty).toHaveBeenCalledWith('--photo-booth-height', '1080px');
      });
    });
