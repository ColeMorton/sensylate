...
describe('Error Handling and Edge Cases E2E Tests', () => {
  ...
  describe('Export Error Scenarios', () => {
    ...
    it('handles export API failure gracefully', async () => {
      ...
      await page.waitForSelector('text=Export generation failed', { timeout: 10000 });

      await e2eHelper.takeScreenshot(page, 'export-api-failure-error');

      // Export button should be enabled again
      const exportButtonAfterError = await page.$('button:has-text("Export Dashboard")');
      const isDisabled = await exportButtonAfterError?.evaluate(el => (el as HTMLButtonElement).disabled);
    ...
    it('prevents multiple simultaneous exports', async () => {
      ...
      const isDisabledDuringExport = await exportButton.evaluate(el => (el as HTMLButtonElement).disabled);
  ...
  describe('Invalid Parameter Handling', () => {
    it('handles malformed URL parameters gracefully', async () => {
      ...
      for (const params of malformedParams) {
        ...
        const formatValue = await page.$eval('#format-select', el => (el as HTMLSelectElement).value);
        expect(['png', 'svg', 'both']).toContain(formatValue);

        await e2eHelper.takeScreenshot(page, `malformed-params-${btoa(params).substring(0, 10)}`);
      }
    });
    ...
    it('handles missing dashboard gracefully', async () => {
      ...
      await page.goto(`${baseURL}/photo-booth?dashboard=nonexistent_dashboard`, {
        waitUntil: 'networkidle0',
        timeout: 15000
      });
    ...
    it('handles extremely large viewport dimensions', async () => {
      ...
      await page.goto(`${baseURL}/photo-booth?dashboard=portfolio_history_portrait&aspect_ratio=16:9`, {
        waitUntil: 'networkidle0',
        timeout: 20000
      });
  ...
  describe('Content Security and XSS Prevention', () => {
    it('prevents XSS through URL parameters', async () => {
      ...
      for (const payload of xssPayloads) {
        await page.goto(`${baseURL}/photo-booth?dashboard=${encodeURIComponent(payload)}`, {
          waitUntil: 'networkidle0',
          timeout: 15000
        });
        ...
        await e2eHelper.takeScreenshot(page, `xss-prevention-${btoa(payload.substring(0, 10))}`);
      }
    });
