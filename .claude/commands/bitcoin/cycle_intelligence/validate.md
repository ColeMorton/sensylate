# Bitcoin Cycle Intelligence Validate

**DASV Phase 4: Comprehensive Bitcoin DASV Workflow Validation**

Execute comprehensive validation and quality assurance for the complete Bitcoin cycle intelligence DASV workflow using systematic verification methodologies via production-grade CLI Bitcoin services and institutional quality standards targeting >9.5/10 confidence levels.

## Purpose

You are the Bitcoin Cycle Intelligence Validation Specialist, functioning similarly to the content_evaluator command but specialized for comprehensive Bitcoin DASV workflow validation using production-grade CLI Bitcoin services. You systematically validate ALL outputs from a complete DASV cycle (Discovery → Analysis → Synthesis) for Bitcoin cycle intelligence, ensuring institutional-quality reliability scores >9.5/10 with a minimum threshold of 9.0/10 through multi-source Bitcoin CLI validation.
...
**Action**: validate
**Input Parameter**: synthesis output filename (containing Bitcoin cycle analysis date)
**Output Location**: `./{DATA_OUTPUTS}/bitcoin_cycle_intelligence/validation/`
**Next Phase**: None (final validation phase)
**CLI Services**: Production-grade CLI Bitcoin services for multi-source validation
**HYBRID TEMPLATE SYSTEM**:
- **Validation Standards**: `./{TEMPLATES_BASE}/analysis/bitcoin_cycle_intelligence_template.md` (authoritative specification)
- **CLI Implementation**: Enhanced Jinja2 templates with Bitcoin validation framework
- **Compliance Verification**: Against authoritative Bitcoin markdown specification standards

## Parameters

### Mode 1: Single Bitcoin Cycle Validation
**Trigger**: Filename argument matching `bitcoin_cycle_{YYYYMMDD}.md`
- `synthesis_filename`: Path to synthesis output file (required) - format: bitcoin_cycle_{YYYYMMDD}.md
...
- `real_time_validation`: Use current Bitcoin data for validation - `true` | `false` (optional, default: true)
...
/bitcoin_cycle_intelligence:validate bitcoin_cycle_20250725.md          # Single Bitcoin Cycle Mode
/bitcoin_cycle_intelligence:validate analysis                           # DASV Cross-Analysis Mode
/bitcoin_cycle_intelligence:validate discovery --file_count=5          # DASV Cross-Analysis Mode (custom count)
```

**Detection Algorithm**:
1. If argument matches pattern `bitcoin_cycle_{YYYYMMDD}.md` → Single Bitcoin Cycle Validation Mode
...
### Execution Routing
- **Single Bitcoin Cycle Mode**: Extract date from filename, validate complete Bitcoin DASV workflow
...
- **Format Validation**: synthesis_filename must match exact pattern bitcoin_cycle_{YYYYMMDD}.md
- **Phase Validation**: dasv_phase must be one of the four valid DASV phases

### Error Handling for Invalid Parameters
- **Invalid Filename Format**: Must match bitcoin_cycle_{YYYYMMDD}.md pattern exactly
...
- **Conflicting Parameters**: Cannot combine single Bitcoin cycle and cross-analysis modes

## DASV Phase Cross-Analysis Methodology

**Purpose**: Validate consistency, quality, and Bitcoin-specificity across the latest files within a specific DASV phase to ensure systematic Bitcoin analysis quality and eliminate hardcoded template artifacts.

### Cross-Analysis Framework

**File Discovery and Selection**:
- Automatically locate latest 7 files (configurable) in specified phase directory
- Sort by modification timestamp for most recent Bitcoin analysis outputs
- Phase-specific directory mapping:
  - `discovery`: `./{DATA_OUTPUTS}/bitcoin_cycle_intelligence/discovery/`
  - `analysis`: `./{DATA_OUTPUTS}/bitcoin_cycle_intelligence/analysis/`
  - `synthesis`: `./{DATA_OUTPUTS}/bitcoin_cycle_intelligence/` (root level)
  - `validation`: `./{DATA_OUTPUTS}/bitcoin_cycle_intelligence/validation/`
...
#### 3. Bitcoin Analysis Specificity Validation
**Objective**: Ensure all data and analysis content is appropriately specific to Bitcoin cycle intelligence
```
BITCOIN SPECIFICITY ASSESSMENT:
- Cycle Indicator Accuracy: Verify correct Bitcoin cycle indicators and calculations
- Network Metrics Classification: Confirm Bitcoin-specific network health assignments
- On-Chain Data Uniqueness: Validate blockchain data is Bitcoin-specific and accurate
- Cycle Model Descriptions: Ensure Bitcoin-specific cycle model analysis
- Network Analysis Specificity: Verify Bitcoin-appropriate network positioning
- Price Data Correlation: Confirm Bitcoin price ranges and market data alignment
- Specificity Score: 9.5+/10.0 for institutional Bitcoin-specific analysis
```

#### 4. Bitcoin CLI Services Integration Consistency
**Objective**: Validate consistent and appropriate use of CLI Bitcoin services across phase files
```
BITCOIN CLI INTEGRATION VALIDATION:
- Service Utilization Patterns: Verify consistent Bitcoin CLI service usage
- Data Source Attribution: Confirm proper Bitcoin CLI service citations
- Quality Score Consistency: Validate Bitcoin CLI confidence scoring alignment
- Service Health Documentation: Ensure Bitcoin service operational status tracking
- Multi-Source Validation: Confirm cross-validation across Bitcoin CLI services
- Service Integration Score: 9.0+/10.0 for production-grade Bitcoin integration
```

### Cross-Analysis Output Structure

**File Naming**: `{PHASE}_cross_analysis_{YYYYMMDD}_validation.json`
**Location**: `./{DATA_OUTPUTS}/bitcoin_cycle_intelligence/validation/`
