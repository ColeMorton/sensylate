...
def test_live_monitor_generation():
    ...
    for watch in monitor['watch_list']:
        print(f"  {watch['ticker']}: {watch['return']:+.1%} return, {watch['mae']:.1%} MAE ({watch['risk_level']} risk)")
...
def validate_synthesis_schema():
    ...
    try:
        ...
        required_props = [
            "portfolio", "synthesis_metadata", "report_generation_status",
            "executive_dashboard", "internal_trading_report",
            "live_signals_monitor", "historical_performance_report",
            "content_validation", "next_phase_inputs"
        ]
